...
`.github/workflows/ci.yml` is the one build workflow. A push to `main` publishes the release `autobuild-{sha}`; a pull request (from someone with write access, or via manual dispatch with `pr_number`) publishes the prerelease `autobuild-preview-pr-{n}-{sha8}` and links it from a comment on the pull request. Either way it builds every lane and publishes them as one GitHub release. A lane is one `<label>.tar.gz`, e.g. `bun-webkit-linux-amd64-lto`: Linux glibc/musl, macOS, Windows, FreeBSD and Android, x64/arm64, release/LTO/debug/ASAN. Everything except Linux and the Windows arm64 debug lane is cross-compiled from a Linux x64 runner.

The lanes are defined in `.github/scripts/plan.mjs` and nowhere else: label, runner, release script, the settings passed to that script, and whether the lane is tested. `node .github/scripts/plan.mjs` lists them (`--json` for the full settings). To add, remove or change a lane, edit `platforms` there. The workflow's `plan` job works out what is being built (commit, release tag, prerelease or not) and turns that table into the matrices of `build` (lanes that are only built), `build-tested` (lanes that are also tested; same steps) and `test`, and into the list of assets the `release` job requires.
