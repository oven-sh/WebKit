...
* **Out-of-line `PutByIdReplace` at the call site** (r14, history §31): the flag-on `put_by_id` inline fast path stores inline offsets directly and, where the owner test can run without a scratch register (x86-64: xor the TID tag from thread-local storage as a memory operand into the loaded word - an owner-tagged SW=0 word xor the tag IS the untagged pointer), out-of-line offsets too; foreign / shared-written / segmented words dispatch to the chain. Where that form does not exist an out-of-line Replace is never `setInlinedHandler`ed (it goes into the chain, whose `putByIdReplaceHandler` carries the full predicate) - inlining it left the chain without the handler and every such put on the optimize slow path for good (measured 10x).
...
* **Read**: load tagged; top16==0xFFFF->segmented read (dependent load through the spine; R3 op or inline); SW=1∧AS->locked slow path (absent under (a)/(c)); else mask, proceed as today. No TID check. GIL-off Int32-mode element READS (OM I41, r13 here = OM r17): the loaded lane is verified `isInt32` before it is typed Int32 (DFG/FTL `GetByVal`/`ArrayPop` exit BadType; LLInt/Baseline hand back the JSValue untyped already); DFG `ArraySlice` on an Int32-mode source allocates a Contiguous-labelled result GIL-off. GIL-on/flag-off emit nothing new (history §29).
...
* **Transition** (= OM E4 EXACTLY incl. its PA exclusion; D9; r10=OM r12 per-object keying; r13=EMITTED, OM r15): legal iff (compile-time) BOTH source TTL sets valid+watched — the stub/plan registers on the source's `transitionThreadLocal`+`writeThreadLocal` AND the target's (four watchpoints; fire=>§5.3 jettison/stub retirement inside the firing stop) — AND source not ArrayStorage-shaped (OM I31: AS relayouts are cell-locked) AND target `outOfLineCapacity`==source's (non-reallocating: inline offset, or an out-of-line offset within existing capacity; reallocating transitions=R3 until a tagged-butterfly install is specified) AND (runtime) `!isPreciseAllocation(cell)` (bit-test `cell & 8`->R3) AND, after ONE load of the tagged word: ONE owner test for every word (OM r16 N1-I): `((tagged ^ g_jscButterflyTIDTag) & butterflyTagMask)==0` ((currentTID,0)=instance owner, butterfly-less words included; no structure-TID compare); then, in the inline caches (r14 = OM r17 E4-C), in a GIL-off process BOTH legs are claim-first (GIL on, OM r19 E4-G, neither leg claims: store value, [storeStoreFence non-x86], store32 `S'.id`; the (re)allocating form stores nuked(S), fences, publishes the tagged word, fences and stores S', with no re-check): `lock cmpxchg` m_structureID `S.id -> nuke(S.id)` (failure->R3, nothing written), [butterfly-bearing: re-load+mask the word — it cannot move while an owner-tagged SW=0 instance's lane is held], store value (inline slot, or masked-butterfly out-of-line), [storeStoreFence non-x86], store32 `S'.id`. Because both legs claim, the stub watches NO thread-local set and stays valid after the source's sets fire (`tryCachePutBy` no longer refuses fired sources; it still refuses ArrayStorage/copy-on-write sources and reallocation); a butterfly-less transition to an OUT-OF-LINE offset is a first butterfly install (N3): see the (RE)ALLOCATING form. The megamorphic store probe (GIL-on, history §30) uses the same claimed sequence with RUNTIME refusals (PA bit, CopyOnWrite bit, shape >= ArrayStorage) since it is structure-agnostic. (RE)ALLOCATING form (r14, history §31): the shared transition handlers run the owner predicate (only the two free scratches touched, so a failure still falls through with operands intact), allocate + fill the grown storage (GIL off through the thread's TLC slot), store the value INTO it, claim (`lock cmpxchg` S -> nuked S; lost => the operation), re-check the word is still this owner's SW=0 word (a foreign first write may have flipped it and stored into the old storage after the copy; moved => un-claim, the operation), publish the TAGGED word, fence, store S'; the allocation-failure / lost-claim operation (`operationReallocateButterflyAndTransition`, and `operationPutByTransitionReallocatingConcurrent` for per-case-compiled stubs whose handler carries no transition fields) completes the put through the OM protocols (`tryCompleteCachedTransitionConcurrent`, generic add on RESTART). DFG/FTL inline the (re)allocating transition under the same four watched sets, in the flag-off node order: `AllocatePropertyStorage`/`ReallocatePropertyStorage` (pure; copies from the MASKED word; GIL off the TLC slot), `PutByOffset` into the fresh storage (a store whose storage child is the transition's own Allocate/Reallocate node uses that child directly: no re-load, no predicate, never exits, and does not clobber exit state - the storage is unpublished), then an `InvalidationPoint` (every park site of the sequence - the allocation, a sunk value's materialization - lies before it: a fire while one of them parked retires the code THERE, before anything is installed), then `NukeStructureAndSetButterfly` = E4's plain publication (nuke, fence, tagged word - no claim: from the InvalidationPoint to `PutStructure` nothing polls, allocates or exits, so no foreign writer or transitioner of the object can be inside its protocol, each having to fire a watched set in a stop first, and no collection can observe the nuked header), then `PutStructure`. GIL off a (re)allocating variant stays out of FTL `MultiPutByOffset` (its allocation parks mid-node with no InvalidationPoint before the install); GIL on nothing fires while a thread allocates. Predicate failure of any leg->R3 slow path (which fires F2 if foreign and retires the emitter). No poll/allocation between the word load and the last store (I16/OM I29). The JIT never implements any other transition semantics. DFG/FTL: the parser inlines a Transition variant only when the four sets are valid and watchable (else the put is a generic `PutById`, whose IC caches the claimed form), plants `CheckTransitionOwner(base)` (the PA test + the one owner test above as an OSR-exiting check, exit kind BadCache; CSE-able, and never hoisted across a node that GIL-off writes JSObject_butterfly — every poll and every parkable node does) before the `PutByOffset`, and `PutStructure` flag-on is the flag-off store32 preceded by a store-store fence (no lane claim: the check, the four watched sets and the trailing invalidation points after parkable nodes make this thread the lane's only writer); objects the same compilation unit allocated and that ObjectAllocationSinking proves non-escaped lose both nodes as today (PhantomNewObject/MaterializeNewObject materialize with the final structure — thread-local by construction).
...
**Length updates on a published butterfly (ninth round, history §46).** GIL off, every RAISE of a published flat butterfly's public length that another thread could race - a store into a hole below the vector length, push, unshift, a MultiPutByVal hole - is the monotone CAS-max: load the length, done if it is already at least the new one, 32-bit CAS, retry; it never lowers the length. The interpreter, the Baseline IC and DFG/FTL code whose threaded plan did not elide the shared-write check inline that loop; E2-elided DFG/FTL code keeps the plain store (every speculated structure's writeThreadLocal set is registered, so a foreign write must fire it under a stop, which jettisons this code before the write lands). The owner of an SW=0 word is no exception: once the set has fired, a foreign writer flips SW with a lock-free DCAS and raises the length with the CAS-max, so the owner's plain store can lower it - the runtime's own raises follow the same rule (`Butterfly::bumpPublicLengthToAtLeast`; `JSArray::pushInline`'s owner leg stored plainly, R9-22's runtime half). A plain raise elsewhere loses a racing thread's raise and hides its element behind the length (I21). LOWERS - pop, shift, the array-length setter (the Baseline IC's handles only shrinks) - stay plain stores in every tier, as in the runtime: a shrink racing a grow is a program-level race (SPEC-objectmodel, review round 3), and routing them to the runtime changed only their cost. ArrayStorage legs are owner-exclusive inline (a foreign AS SW flip takes a per-event stop) and stay plain. The element stores are unchanged (SAB granularity). GIL on and flag off keep every plain store: no other mutator runs inside a poll-free window. Hole stores stay recorded in the array profile (`MayStoreHole`) in every tier and in the runtime's CAS-max, or the optimizing tiers compile in-bounds code for arrays that take hole stores. Cost of the alternative, measured: routing every non-elided leg to the runtime (r9w) cost 3.2-3.9x the instructions of a hole/push/pop loop once the sets had fired, and 3.6x a hole-filling loop in the lower tiers.
...
* **Deferral**: deferred-overload fires (`Watchpoint.h:493-508`); the scope-exit fire (lock-free by construction) runs steps 2-6. **AMENDED (B-relabelrace, history §22)**: flag-on the deferred overload's entry is NOT owner-serialized for every caller (inline original-array `nonPropertyTransition` fires it lock-free; `firePropertyReplacementWatchpointSet` pre-checks `IsWatched` lock-free), so the claim is a CAS `IsWatched`->`IsInvalidated` BEFORE the membership transfer: exactly one racer transfers (splice still under the membership lock); losers return with their deferred set `ClearWatchpoint` (no scope-exit fire — benign, the winner's fire supersedes) and ASSERT the failed CAS's prior state is `IsInvalidated` (states monotonic, so the legitimate race is assert-clean; any other prior — e.g. `ClearWatchpoint` from a precheck-bypassing direct caller — traps exactly as the former entry assert did). `take()` therefore installs `IsWatched` into the deferred set explicitly (the source's pre-claim state), and `takeWatchpointsToFire`'s entry invariant is re-scoped to "source `IsInvalidated`-by-claimant" flag-on / `IsWatched` flag-off (each arm asserts the one state its protocol permits). Flag-off sequence unchanged (assert + take + flip). DIRECT `fireAll`/`fireAllSlow` callers REQUIRED lock-free w.r.t. every §7/cell lock; Task 11 buckets each: lock-free/world-stopped/lock-holding->**M6** (expected empty; table in PR). STWR debug watchdog RELEASE_ASSERTs stop progress, naming the escaped set.
...
* **GC**: `comparand`=RAW word, never dereferenced/visited, no `WriteBarrier`. Legacy mirror=sole GC root/weak ref; `visitWeak`/`unlinkOrUpgrade` read it as today plus null `m_record` on clear/relink. Always-call records unlinked under STW/GC only. `codeBlockToTransfer` is NOT a root (ninth round, history §43; the earlier publish-time pin is withdrawn): every record naming a CodeBlock is on that block's `m_incomingCalls` before any stop that could find it dead, and flag on the End phase unlinks a dead block's incoming calls world-stopped (`CodeBlockSet`), clearing their records; with I16's poll-free window no thread holds such a record across a stop.
...
* **Virtual-call fast path GIL off (r15, history §32).** The r14 GIL-off rule kept a script executable's arity-check mirror (`m_jitCodeFor*WithArityCheck`) null for good (`installCode`/`clearCode` stored null, `entrypointFor` never refilled it), so the thunks' non-null gate sent every virtual call to a JS function through `operationVirtualCall`. r15: GIL off `installCode` PUBLISHES the mirror itself, for the CodeBlock it installs, as its LAST store - after retracting it FIRST and after the (fenced) CodeBlock-slot store; `clearCode` stores null; `entrypointFor`'s lazy refill stays disabled GIL off (a reader-side write that can land after a later install's retraction). Reader (virtual-call, bound-function and LLInt virtual thunks, unchanged code): load mirror (null -> slow path), load CodeBlock, RE-READ mirror, equal -> dispatch with that pair, else slow path (`virtualForWithFunction`, one CodeBlock snapshot). Soundness: every code-retiring slot write (jettison installing the alternative, `clearCode`, `deleteAllCode`) is world-stopped (I8) and the thunks have no poll between the first read and the jump, so a thunk execution overlaps only tier-up installs; each of those stores mirror := null BEFORE its CodeBlock store and mirror := E(new CodeBlock) AFTER it. If the CodeBlock the reader loaded came from an install whose CodeBlock store follows the first mirror read, that install's null store precedes the CodeBlock store and hence the second mirror read, which then sees null or a newer entry; a newer entry equal to the first read would need the same (CodeBlock, tier) entry published twice or two LLInt-tier CodeBlocks (the shared LLInt arity-check entry) installed for one executable without an intervening stop, and neither happens (LLInt -> baseline -> DFG -> FTL installs each publish a distinct entry; a fresh LLInt CodeBlock follows only `clearCode`, stopped). So equal non-null reads pair the loaded CodeBlock with the code that entry names. The recompare is thus load-bearing GIL off (belt-and-braces GIL on); x86-64 TSO orders the loads, weakly-ordered targets are the recorded IT-8 residual. Host executables publish both mirrors once at construction. Flag-off: `installCode` computes a null as before; thunk bytes unchanged.
...
- **Double arrays the DFG would mint only to convert, GIL off (r15, history §34; OM T4-C/T4-P).** `ArraySlice` is not planted on Double-typed sites and does not admit Double arrays GIL off (a slice of a Double source must be a boxed Contiguous copy); `NewArray`/`NewArrayWithSize` compiled from a profile recommending Double record their result in the profile's last-array word and watch the profile's demotion set. Flag-off/GIL-on unchanged.
- **Map/Set `get`/`has` GIL off (r15, history §35).** Inlined again: the FTL probe reads the owner's seqlock version (acquire) first, bounds every followed index by the storage's immutable length and the step count likewise, and re-checks the version behind a load-load fence before its found/not-found answer stands (else the runtime's validated reader); GIL off `LoadMapValue` maps an empty or deleted-sentinel value slot to `undefined`. DFG tier calls the operation. Mutating/iterating hash-table intrinsics remain calls GIL off.
- **Watcher-less Class-A fire (r15, history §36; §5.6).** A code-invalidating set with no members fires by the state flip alone, decided under the membership lock that `add()` also takes (an `IsInvalidated` set refuses members), so no stop is requested for the property-replacement and scope-cache sets ICs arm speculatively.
...
I21. Flag on: polling traps ONLY (`usePollingTraps` forced, M2b; async breakpoint patching=I2 violation); GIL-off, every DFG/FTL poll is immediately followed by an invalidation point (CheckTraps emits one)=>parked mutators resume into the patched exit, never across jettisoned elided code; Task-13 lint extends I16 to poll windows. GIL-on (r13): CheckTraps is modeled and compiled exactly as flag-off with polling traps (InternalState only; no invalidation point, no heap clobber, cloneable for loop unrolling): a thread at a poll holds the VM's GIL and the phase-1 GIL is handed off only inside blocking primitives (api §5.2: join/hold/wait/Atomics.wait), which are calls (world-clobbering, followed by invalidation points); no other mutator of the VM can rewrite a heap fact while this thread is at the poll, and the services that run there (termination, watchdog, debugger, VMManager stop) are the flag-off set with the flag-off invalidation coverage. GIL-off CheckTraps is cloneable too (each clone keeps its own invalidation point and poll).
