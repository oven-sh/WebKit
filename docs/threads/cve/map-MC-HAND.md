...
Date: 2026-06-15 (re-audit; supersedes 2026-06-07). Tree: jarred/threads
(ungil ladder mid-closeout). Specs consulted: SPEC-api rev 14
(§4.6/F5/5.5/5.5a/5.6/5.7), SPEC-ungil + UNGIL-HANDOUT rev 32
(§E.1-E.7, §A.2, annex N6, ANNEX WS1), SPEC-objectmodel rev 14. Read-only
audit; tests under JSTests/threads/cve/ are written; execution status per
CVE-AUDIT-STATUS.md.
...
below. The one surface that earned **susceptible-suspected** at the
2026-06-07 audit (S6) was CLOSED 2026-06-10 by re-deriving the claim verdict
under the same lock section as the table insert; this revision re-audits the
landed shape.
...
Source/JavaScriptCore/runtime/ThreadObject.cpp:476-605.
...
- F1 (SPEC-api 4.6.1): the result Strong is written BEFORE the Phase
  ...
  (ThreadObject.cpp:352-367 writer side; :509, :591-593 reader side). The
  pair {Phase, result} is published as a unit, and Phase transitions
  Running -> Finished|Failed exactly once — completion state cannot be
  observed torn or attributed to the wrong outcome arm.
- F5: the joiner's wait loop re-checks Phase under `joinLock` on every 10 ms
  quantum BEFORE polling termination (ThreadObject.cpp:509-543) — a
  completion observed under the lock takes priority over a concurrent
  termination request — and the writer's notifyAll is under the same lock
  (:364-367), so no lost wakeup.
- Termination surfacing on the loser arm is request-then-throw on the
  joiner's own VM/lite state, never a write into the joinee's ThreadState —
  the cancel can never corrupt the joinee's completion record.
- AB-17 W1: GIL-off, the park polls termination against the PARK lite's word
  only, and the watchdog-check bit is serviced by the W1 reacquisition
  episode with `joinLock` dropped (:528-543) — the loop re-checks Phase
  under the re-taken lock, so a completion that raced the episode still
  wins.
...
cleared only by the 5.10 finalizer hook (ThreadObject.cpp:112-146), so
nothing dangles.
...
(c) The GIL-off joiner-side microtask drain is gated so a non-main embedder
joiner with no current lite cannot drain the main carrier's queue from a
foreign thread — jobs are delayed to their natural turn, never run on a
wrong-owner thread.
...
of which side wins. ThreadObject.cpp:607-678 (registration), :358-367
(completion swap), :154-169 (settle).

**Governing protocol.**
- F5: the phase check and the asyncJoiners append are both under `joinLock`
  (:628-644); the completion sequence's Phase store and asyncJoiners swap
  are both under the same lock (:358-367). A ticket is either swapped out
...
- AsyncTicket::settle is exactly-once by the m_settled CAS
  (ThreadManager.cpp:139); repeat asyncJoin calls mint distinct
...
- Settlement reads `thread->result()` ordered after the Phase release-store
  (:165-167 with the F1 comment); the ticket's dependency vector roots the
...
- Abandoned tickets (receiver never completes: lazy main/embedder tid-0
  ...
  (:112-146), which runs with the JSLock held; drained tickets "were never
  passed to settleJoinTicket ... so no settle task can later read the
  cleared Strong" (:136-138).
...
(`promise->realm()`) — the promise settles in ITS OWN realm regardless of
which thread runs the settle task, the same property the upstream
...
- (i) is closed twice over. First: TIDs are retired forever (SPEC-api
  Deviation 10; ThreadObject.cpp:383 "The TID is retired forever") — there
  ...
  nativeThread pointer, "never TID" (5.7.2; ThreadManager.cpp:1003-1013 —
...
- (ii) the affinity table treats any entry whose Weak is not
  Live-and-equal-to-the-probed-object as ABSENT (ThreadManager.cpp:1016-1049,
  header comment + restrictionOwner stale-key check); restrictObject's
  stale-replace arm destroys the dead predecessor's entry, and the
  finalizer's identity check (context != entry) makes a racing late
  finalizer a no-op (:1086-1099) — a recycled address can neither make a
...
ThreadManager.h:159-331 (§E.3/E.4 protocol docs), ThreadManager.cpp:139-217
(settle + routing), :782-978 (drain loop + close residue).

**Governing protocol (SPEC-ungil §E.3/E.4/E.5, BINDING).**
- Exactly-once: the m_settled CAS (ThreadManager.cpp:139) is won before any
...
- The open-or-fallback decision is made UNDER the registrant's inboxLock
  (settleViaRegistrantRouting, :158-198): open => append + §E.3 rule-1
  keepalive decrement + wake, all in ONE inboxLock section ("no close can
  interleave between a decrement and its append", :164-179); closed => win
  the keepalive CAS but SKIP the dead counter, drop the lock, and route to
  the main fallback via the landed scheduleWorkSoon path. Inbox closure is
  MONOTONIC ("open exactly once pre-fn, false forever at close"), so the
  post-drop fallback can never race a reopen.
- Close residue: tasks already queued when the inbox closes are harvested
  and re-routed to main (closeThreadInboxAndComplete, :782; :208-211 "the
  ticket already won its m_settled CAS when it was enqueued"); ~ThreadState
  RELEASE_ASSERTs both queues empty (ThreadManager.h:307-331).
...
call-site edits land with the E2A wiring, ThreadManager.h:184-196): until it
...
under the list lock" (ThreadAtomics.cpp:1097); notify dequeues and flips
...
(ThreadAtomics.cpp:1213-1214, :1390 — "the dequeued <=> flipped invariant
stays intact"). GIL-off, finite timeouts become registrant-local deadlines
whose expirer must win `tryDequeue` under the list lock first — "an
already-notified/dequeued waiter returns false (the in-flight settle wins;
§E.5 harvest rule)" (ThreadManager.h:276-288). Behind all of it, the ticket
m_settled CAS is a second exactly-once belt, and `consumed` (release-fn /
asyncWait arbitration, ThreadManager.h:127, :253) makes the lock-hold
handoff single-winner (the loser throws the 4.2 Error; the hold epilogue's
...
threadFuncRestrict (ThreadObject.cpp:857-928) runs: step (0) affinity check
(:873-882, one m_affinityLock section inside objectAffinity), THEN the
5.7.1 conversion sequence (:884-913, lock-free w.r.t. the affinity table),
THEN restrictObject's table insert (:923 -> ThreadManager.cpp:1051-1084,
a SECOND m_affinityLock section).

**2026-06-07 finding (now CLOSED).** GIL-off, the check and the claim were
separate lock sections: T_A and T_B both observed Affinity::None at step
(0); T_A's restrictObject inserted; T_B's restrictObject hit the live-entry
arm and returned SILENTLY — the comment "(foreign re-restrict was rejected
by the caller's affinity step (0))" baked in exactly the atomicity the GIL
provided and ungil removed. T_B was told it owned an object the table
assigned to T_A — shared ownership state observed by the wrong logical
owner, memory-safe data exposure: the MC-HAND definition verbatim.

**Landed fix (2026-06-10, CVE-AUDIT-STATUS.md).** `restrictObject` now
returns bool and re-derives the claim verdict under ITS OWN lock section:
the live-entry arm returns `entry->owner.ptr() == &owner`
(ThreadManager.cpp:1061-1073 — "the claim verdict is therefore re-derived
under THIS lock section: owner identity is the ThreadState pointer (5.7.2),
compared, never dereferenced"); `threadFuncRestrict` surfaces the frozen
4.1 ConcurrentAccessError on a false return (ThreadObject.cpp:918-926 —
"the loser surfaces the frozen 4.1 re-restrict ConcurrentAccessError
(MC-HAND S6)"). The check-then-claim TOCTOU is now closed by re-checking
under the SAME lock section as the insert; step (0) remains a fast-path
early-out only.

**Adversarial self-check on the landed shape.**
(a) Loser ran the 5.7.1 conversion sequence on the winner's object before
losing: explicitly noted at :919-922 as "object-model-safe (M5) and
perf-only (the conversions are exactly what the winner's restrict
performs/requires anyway)"; switchToSlowPutArrayStorage is idempotent on
already-SlowPut shapes (:896-900 — "the call must then no-op, not CRASH").
No data exposure: the loser stored nothing in o, and o's resulting shape is
identical to a single-winner restrict.
(b) Stale-replace arm (recycled address, dead predecessor entry): returns
true and installs the caller as owner — correct, the predecessor is dead
and the caller IS the first restrict of the live object; the predecessor's
late finalizer is identity-checked into a no-op (:1086-1099).
(c) Three-way race (T_A wins insert; T_B and T_C both hit the live arm):
both compare `owner.ptr() == &owner` against T_A's entry under the lock,
both get false, both surface CAE — no phantom claim.

**Verdict: immune-by-construction** (claim verdict re-derived under the
m_affinityLock section that holds the insert; owner identity = ThreadState
pointer, unforgeable). Regression-pinned by
`JSTests/threads/cve/mc-hand-restrict-claim.js` (20/20 GIL-off since
2026-06-10 per CVE-AUDIT-STATUS.md): two threads race Thread.restrict on a
fresh object per round, then re-probe ownership after a barrier. Oracle per
round: exactly ONE racing restrict succeeds and the other throws
ConcurrentAccessError; the settled table owner must be the thread whose
racing call reported success.
...
to a heap §10 stop (ArrayBuffer.cpp:180-263 annex N6 quarantine block;
:231-232 "of any number of racing detach()/transferTo() calls EXACTLY ONE
moves ownership into the quarantine and fires notifyDetaching — no double
free"), so a stale old-owner read sees stale-but-mapped SOURCE bytes, never
the new owner's. There is no completion state the wrong owner can observe —
the "completion" (the new buffer) is a fresh allocation.
...
|---|---------|--------|---------------------|---------|
| S1 | join: completion vs termination | ThreadObject.cpp:476-605 | F1/F5; AB-17 W1; TERM1.2 (VM-granular cancel) | immune-by-construction |
| S2 | asyncJoin register vs complete | ThreadObject.cpp:607-678, :358-367, :112-146; ThreadManager.cpp:139 | F5 append-vs-swap under joinLock; m_settled CAS; I12/I20; 5.10 | immune-by-construction (S4 test covers end-to-end) |
| S3 | ownership identity vs recycled TID / cell address | ThreadObject.cpp:383; ThreadManager.cpp:1003-1049, :1086-1099 | Dev 10 (TID retired forever); 5.3/5.7.2 pointer identity; Weak-identity checks | immune-by-construction |
| S4 | ticket settle vs registrant death / inbox close | ThreadManager.cpp:139-217, :782-978; ThreadManager.h:159-331 | SPEC-ungil §E.3/E.4/E.5 (m_settled CAS, inboxLock-arbitrated routing, monotonic close, main fallback) | needs-test → mc-hand-dead-registrant-settle.js |
| S5 | property-waiter notify vs timeout vs sweep | ThreadAtomics.cpp:1097, :1213-1214, :1390; ThreadManager.h:276-288 | 5.6 flip-once-under-LL; dequeue<=>flip; §E.5 harvest rule; consumed/G22 | immune-by-construction |
| S6 | Thread.restrict claim vs claim | ThreadObject.cpp:857-928; ThreadManager.cpp:1051-1084 | SPEC-api 4.1 "re-restrict => CAE" — claim verdict re-derived under the insert's m_affinityLock section (LANDED 2026-06-10) | immune-by-construction (regression-pinned by mc-hand-restrict-claim.js) |
| S7 | ArrayBuffer transfer handoff | ArrayBuffer.cpp:180-263 (annex N6 quarantine) | annex N6 arm 2 (copy+detach; handle-move rejected r14); exactly-one detach winner | immune-by-construction (existing P6/S2 tests) |
...
- JSTests/threads/cve/mc-hand-restrict-claim.js — `--useJSThreads=1`
  (regression pin for the S6 fix; PASSING 20/20 GIL-off since 2026-06-10)
- JSTests/threads/cve/mc-hand-dead-registrant-settle.js — `--useJSThreads=1`
  (S4 fidelity probe; deterministic ordering)
...
bound every wait (annex T2 conventions).
