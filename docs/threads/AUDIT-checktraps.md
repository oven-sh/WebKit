...
7. **P10c park class — CLOSED for the chartered allocation widening (amend
   ...
   for the residual is recorded in P10c-R. (Eleventh session: the tree's
   predicate already covers the long tail through its `default:` arm, see the
   correction in the P10c-R row; what that costs and the proposed exclusion of
   call-free nodes such as `PutStack` are in DESIGN-PROPOSALS B-3 / D-B4.)
