# SPEC-heap.md - FROZEN implementation spec (rev 14)
...
4. "Concurrent-GC protocol extends to N mutators" - deferred. Shared mode is synchronous, conductor-driven (§10); disabled: concurrent marking, collector-conn, incremental assist, activity-callback collection, mutator-concurrent sweeping; parallel marking inside the stop stays. Option off=>today's protocol (I10). (Superseded in part: GIL off one concurrent window per cycle is live, SPEC-congc §7.1a; a shared server's conductor waits that window out with its access released, which with one mutator means nobody runs during it - DESIGN-PROPOSALS J1.)
...
- F8. **Stop-pending vs. access-acquire (Dekker pair, normative).** GSP; sole writer the conductor: seq_cst `true` at §10.3, `false` at §10.8. Client AHA: (0) already HasAccess (`m_accessOwner`)=>return (**idempotent, no CAS-spin**: JSLock recursion/ACT/hook re-entry); (1) seq_cst CAS NoAccess->HasAccess; (2) seq_cst load GSP; (3) if pending, **mandatory revert** - seq_cst exchange->NoAccess, signal GBC under GBL, block until `!GSP`, retry from (1). Conductor: seq_cst samples each client under GBL until all NoAccess. **seq_cst mandatory** - acq/rel misses both store-load pairs (proof: history). **Conductor re-entry (ninth round, history §36):** while the conductor's own window is open - from its §10.3 GSP store to its §10.8 clear, tracked by a flag local to the conductor's thread (no other thread reads conductor state) - its own client's AHA skips (3) and the Mode-machine leg (§A.3.2b(i): a GC stop raises the VMM stop for every VM, and a thread that lost the election participates in it, so that leg gates fresh access - parking the conductor there would wait for its own window's close) and takes access: the destructors and weak finalizers it runs inside the window (§10G) may take the VM's API lock, and every `JSLock::lock` GIL off runs the gated AHA, so the revert would wait for a clear only the conductor performs. A conductor that re-entered keeps access while the API lock is held recursively; a non-final close releases it (the Reentry open requires the conductor access-released); after the final close the conductor's re-acquire is the (0) no-op.
...
7. **Collection+safepoint work** - full synchronous collection per §10B; then, still stopped, the §11 reclaim sequence (I11) and the §10E block-retention check.
...
### 10E. Block retention at cycle end (r14; history §28)

Physical reclamation of empty marked blocks is world-stopped only once shared (§11's rule for freed memory: sibling mutators' lock-free reads may touch dead-verdicted cells), so the shared server's only steady-state reclamation point is the end of a conducted cycle, still stopped (step 7). Policy there, two arms. FULL cycles that were REQUESTED (the request names the Full scope: the embedder's gc(), an idle or memory-pressure collection, the shell's; eighth round - an allocation-paced cycle the heap itself upgraded to Full takes the eden arm) keep the sixth round's slack rule: if committed capacity exceeds 1.5x of `max(sizeAfterLastCollect, minHeapSize)` - the live size the Full collection just measured - run the synchronous sweep (destructors, so destructible and weak-bearing empty blocks become reclaimable) and free every empty block; this is where a program that went quiet gets its memory back, and an embedder whose heap is mostly destructible cells depends on it (eden cycles never sweep those blocks, so the eden arm below cannot free them). EDEN cycles, which carry the steady-state allocate-collect churn: RETAIN the empty blocks a cycle leaves - they are the next cycle's allocation supply - and shed what the program stops using gradually. Let `working = max(sizeBeforeLastCollect, m_maxHeapSize, minHeapSize)`: the heap size at the start of the cycle that just ran, Eden or Full (live data plus the eden the next cycle fills again; eighth round - keying it on the last EDEN start after an allocation-paced Full shrank to the mid-cycle size and re-minted the difference every Full/Eden pair), the allocation budget just recomputed by `updateAllocationLimits`, and the floor. If `capacity() <= working + working/4`, free nothing. Otherwise free empty, non-destructible, not-in-use blocks (BVL held per directory, the free itself outside it, exactly `BlockDirectory::shrink`'s discipline) until capacity is down by HALF of `capacity() - working` (or none is left): a steady allocate-collect oscillation keeps its blocks, a peak the program does not return to (the run-up to a Full collection, a wide parallel phase followed by a narrow one) decays geometrically over the following cycles instead of being held (the seventh round measured the alternative - holding the pre-Full peak - at 3.7x the resident set of the previous policy on the scaling suite's splay-like at eight threads; the decay form is within 1.2x of it while JetStream's splay still mints a third of the blocks it did). No synchronous whole-heap sweep runs from this point; `shouldSweepSynchronously()` (critical memory, mini mode, the option) keeps its finalize-time sweep as flag-off. Rationale: a rule keyed on `capacity() > m_maxHeapSize` alone fires after nearly every cycle (the blocks that held the cycle's eden garbage are exactly the excess) and `shrink()` frees every empty block, so each cycle re-mints its eden through the block allocator and first-touch faults (measured 5x-56x the non-shared block count per benchmark, PERF-RESULTS §6.4 item 1); the non-shared heap frees empties only from the idle-time sweeper. The 1.5x band bounds committed capacity against the budget, which is the decommit guarantee the earlier every-cycle rule was introduced for.

Eighth round, second amendment: the eden arm never frees a block the cycle that just ended allocated into (each directory keeps a copy of its Eden set taken when the collection ended, `edenLastCycle`); only empty blocks OUTSIDE that set count as surplus against the budget. The blocks a steady program fills and empties every cycle are exactly the ones its next cycle needs, whatever the capacity arithmetic says; without this a single larger cycle (§10F's doubled allowance while a second thread was briefly allocating) left surplus that the budget rule then shed from the wrong directories every cycle and the allocator re-minted (12,600 blocks over a steady phase that mints 150).

Ninth round, third amendment (history §32; LIMITATION found in the eleventh session: the per-stop budget of 32 bounds the backlog only for a fixed population of such blocks. Every Structure block is weak-bearing, the allocation path cannot reuse an unswept one, and a program that makes more than 32 per cycle grows without bound: `scaling/string-heavy.js` at ONE thread makes about 200 per Eden cycle, capacity ramps from 26 to 372 MB over 132 cycles with under 10 MB live, 478-498 MB resident against 88 MB flag off, 141-145 MB with a budget of 256 or none; DESIGN-PROPOSALS J6 / D-J2): the end of every conducted cycle also sweeps, inside the stop, the unswept weak-bearing blocks (blocks whose WeakSet holds WeakBlocks). Every sweep that runs alongside mutators skips them - the synchronous collection's tail, the allocation path, the empty-block steal, the IncrementalSweeper - because it would race another client's lock-free `Weak<>` deallocation and run weak finalizers under that client; they were left for "the next world-stopped sweep", and the only one in steady state was the requested-Full slack arm above. A program that never requested a Full collection kept every block a `Weak<>` had pointed into, with its dead cells never destroyed and its memory never reclaimed (`gc-stress/destructors-run-once-on-any-thread.js`: 12,350 of 50,000 destructible cells never destroyed after twenty synchronous collections, one block per round - the block that also held the round's `Thread` wrapper, which a `Weak<>` names). The sweep of those blocks now runs in the stop; its cost is in PERF-RESULTS (ninth round). Amended (history §35): a cycle end sweeps at most `sharedGCWeakBearingSweepBudget` weak-bearing blocks (default 32; 0 means all), continuing from where the previous cycle end stopped - a cursor over the directories and one over each directory's blocks - so with n such blocks each is swept within ceil(n / budget) cycle ends; a collection somebody requested (the request names a scope: the embedder's gc(), a memory-pressure or idle notification, `$vm`) sweeps all of them. A Full cycle makes every block unswept again, so without the budget back-to-back Full cycles re-swept every weak-bearing block each time.

### 10F. Eden allowance with several allocating clients (r15, eighth round)

The allocation trigger (`collectIfNecessaryOrDefer`: bytes allocated this cycle against `m_maxHeapSize - m_sizeAfterLastCollect`) budgets ONE nursery, sized as flag-off from the live heap. k clients allocating in parallel exhaust it k times as fast, and every eden collection is a stop of all clients, so the number of stops per second of wall time grew with the thread count while the work each stop does (roots, survivors, remembered set) did not shrink: the seventh round measured raytrace-like at four threads spending 47 ms of a 275 ms run in 63 eden pauses, the ceiling behind its 2.9x. Rule: when the server is shared and memory is not critical, the cycle's allowance is the budget above plus `(k - 1) * min(m_maxEdenSize, largeHeapSize)`, where k is the cycle's distinct-allocating-client count of §10E/F4-burst (a client counts once it has allocated a quarter of a fair eden share this cycle, so parked or barely allocating threads add nothing). The count is read live, so the allowance grows during the cycle as clients join; Full-collection triggers, the critical-memory clamp and every end-of-cycle computation are unchanged, and k = 1 (or a non-shared heap) is byte-identical to before. Cost: up to one capped nursery of additional floating garbage per additionally allocating thread.

### 10G. Destructors and finalizers: which thread (ninth round; history §31)

A dead cell's destructor, a `WeakHandleOwner::finalize`, and a `Heap::addFinalizer` callback run on the thread that sweeps the cell's block or precise allocation. Flag off that is the VM's own thread (or whichever thread holds its API lock). With the flag on it can also be:

- GIL on: any JS thread that holds the lock while it allocates from the block (the lazy sweep), services a collection's epilogue (the eager sweep of precise allocations every cycle, which holds up to `numberOfLowerTierPreciseCells` cells of every type; the synchronous sweep under critical memory), or runs a synchronous collection (the shell's `gc()`, an embedder's `collectNow(Sync)`).
- GIL off, in addition: the collection's conductor inside the stop (the epilogue, §10E's requested-Full sweep, weak finalizers), the same conductor after the stop for deferred lambda finalizers, and any client's synchronous-collection tail outside the stop (non-weak-bearing blocks, while the other clients run).

A spawned thread's exit sweeps nothing (its blocks go back unswept), the IncrementalSweeper runs on the VM's run-loop thread, and `lastChanceToFinalize` runs on the thread that destroys the VM. Every destructor runs exactly once, whichever thread that is.

Contract for embedders. A destructor or finalizer must not assume which thread runs it. It may read the dying cell and free what the cell owns. Teardown with thread affinity - handles into per-thread or per-event-loop structures, event-loop or I/O state, thread-local lookups - must be handed to the owning thread. The sweeping thread holds the GIL (GIL on) or runs with the other clients stopped or alongside them (GIL off), so a destructor must not take a lock that another thread may hold across a safepoint, allocate a cell, or enter JS. It may take the VM's API lock (a `JSLockHolder`), recursively as on main: GIL off the conductor that runs it inside its stop window re-enters its own access for that (F8 conductor re-entry). JSC does not marshal teardown itself: which native parts are thread-affine is known only to the embedder (Bun posts its generated classes' native finalizers, and releases of its strong-handle slots, to the VM's event loop when they run on another thread; the delay is the same kind a lazy sweep already has, and the posted work receives only the native pointer, never the dead cell). Test: `gc-stress/destructors-run-once-on-any-thread.js` (`$vm.createDestructionProbe`).
...
## ANNEX B — Per-client allocation attribution & soft heap quota (CVE-B13 / MC-DOS S3)

**Status: post-r13 chartered design proposal; adversarial review r1 (2026-06-18) amendments applied — B.1 `Atomic<>`/hoist-above-latch, B.3 trap-checkpoint reroute (no Assert-mode null). FROZEN pending r2. No engine behaviour lands at charter time — the `maxJSThreadHeapBytes` knob is stubbed at 0 (inert) and every mechanism below is ISS-gated, so flag-off remains byte-identical (I10).**

**Problem (availability-only).** Under ISS, N clients allocate into one server with one set of GC trigger/criticality thresholds. There is no per-client quota and no per-client attribution: a single spawned thread in a tight allocation loop drives the shared heap to its limit, and the resulting OOM lands on whichever client allocates at the threshold — including the main thread or an innocent sibling. Single-thread parity does not apply: in every shipping JSC config an agent wanting isolation gets its own Heap; `--useJSThreads` collapses N agents onto one. Integrity is unaffected (allocation-failure paths are today's); the defect is blast-radius + misattribution. (Source: docs/threads/cve/map-MC-DOS.md S3.)

**B.1 Per-client gross-allocation counter (attribution).** New `GCClient::Heap` member `Atomic<size_t> m_grossBytesAllocated { 0 }`, declared adjacent to `m_bytesAllocatedThisServerCycle` (Heap.h:2602). Own access-holding thread is the only writer (relaxed `fetch_add`); B.2 reads it cross-thread while the writer runs, so `Atomic<>` (relaxed) keeps the TSAN-zero gate clean at zero slow-path cost — same shape as the adjacent `Atomic<uint64_t> m_localEpoch` (Heap.h:2603). Charged in the existing `didAllocate` ISS leg (Heap.cpp:4138-4145): hoist the `if (client)` null-guard (collector-thread world-stopped allocations bind no client) and bump **independent of** the F4-burst `m_allocatedThisServerCycle` latch — that latch sets true in steady state, so gating the bump on it would stop accumulation for exactly the heavy allocators B13 targets. Never reset (monotone since ACT). Zero fast-path cost: `didAllocate` is reached only from `allocateSlowCase` block-acquisition / precise / `reportExtraMemory` paths, exactly the §5.2/§5.6 slow-path charging points the S3 audit named; the inline bump-pointer fast path never calls `didAllocate`. Precise allocations and `reportExtraMemoryAllocated` flow through the same site (Heap.cpp:821), so oversized/extra-memory pressure is attributed identically.

**B.2 OOM attribution (always-on once ISS).** New server helper `void Heap::formatClientAttribution(StringBuilder&)`: under `clientSet().registryLock()`, walk live clients, read each `m_grossBytesAllocated` (relaxed atomic load — diagnostic precision only), and emit the top-K (`K=4`) as `"[jsThread tid=<n> alloc=<bytes>]"` using the api-spec TID surface (VMLite/ThreadManager), main client labelled `tid=main`. Called from the ISS OOM-reporting seam: `JSC::Heap::didExceedMaxLiveSize()` / `reportCrashInfo` and `ErrorInstance` OOM-message construction (manifest item, runtime-side; out of heap's owned paths — recorded in INTEGRATE-heap.md alongside §13). No quota required for attribution; this leg is unconditional under ISS so an embedder that leaves the quota at 0 still gets the diagnostic.

**B.3 Soft quota (opt-in).** `Options::maxJSThreadHeapBytes()` (Size, default 0 = disabled). When non-zero AND ISS, the B.1 charging site additionally evaluates `client->m_grossBytesAllocated - client->m_grossBytesAtLastCycleEnd >= Options::maxJSThreadHeapBytes()`: i.e. the quota bounds **per-client gross allocation since the last completed GC cycle**, not lifetime gross (a long-lived well-behaved thread must not trip it) and not live footprint (per-client survivor accounting would require per-client mark attribution — explicitly out of scope; block-granular live charging at TLC handout/return was considered and rejected: steals + conductor-side stop/resume make the credit side cross-thread, forcing atomics on a path that today is plain). `m_grossBytesAtLastCycleEnd` is a second per-client `size_t` snapshotted in the existing world-stopped `updateAllocationLimits` `clientSet().forEach` reset loop (Heap.cpp:4012) — same site that already zeroes `m_bytesAllocatedThisServerCycle`, so no new stop-phase walk. On quota exceed: set per-client sticky `bool m_heapQuotaExceeded` AND fire that client's VMLite trap word (VMTraps `NeedWatchdogCheck`-class, per-lite §A.2.1 path; out-of-tree manifest hunk). `LocalAllocator::allocateSlowCase` (inside the MSPL section, after CIND returns) and `CompleteSubspace::tryAllocateSlow` consult the sticky bit via `currentThreadClient()` and early-return `nullptr` **only when `failureMode == AllocationFailureMode::ReturnNull`** — an in-flight `Assert`-mode allocation MUST succeed: returning null there reaches LocalAllocator.cpp:379 `RELEASE_ASSERT_RESOURCE_AVAILABLE` and crashes the whole process at the per-thread threshold (the opposite of containment), and `throwOutOfMemoryError`'s own `ErrorInstance` allocation is itself Assert-mode. The throw is delivered at the next trap checkpoint instead: `VMTraps::handleTraps` observes the sticky bit and raises `RangeError` with the B.2 attribution suffix + `"per-thread heap quota (maxJSThreadHeapBytes=<v>) exceeded"`. Net: ReturnNull-mode callers fail immediately; Assert-mode callers complete the in-flight cell then throw at the checkpoint — soft-fail only, never `RELEASE_ASSERT`; rule-2 fail-stop remains the existing whole-heap limit, this knob narrows blast radius to the offending client. The sticky bit and trap clear at the same `updateAllocationLimits` snapshot point (the client survived a full cycle; immediate re-offence re-trips next slow path).

**B.4 Non-goals / invariants.** (a) No fast-path cost: every read/write added is on an already-slow path under ISS; flag-off the ISS branch is not taken (I10 holds by construction). (b) No new locks: B.1/B.3 writes are own-thread; B.2's registry walk reuses `clientSet()`'s existing lock (rank 5, §6). (c) No GC-trigger coupling: the quota does NOT feed `shouldDoFullCollection`/eden thresholds — it is a per-client allocation-failure gate only; GC scheduling remains the F4-burst/§31 mechanism. (d) Standalone harness clients (§12.1) participate identically (no `vm()` dependence; TID label degrades to the client pointer when no VMLite is bound).

**B.5 Manifest deltas (recorded into INTEGRATE-heap.md at implementation time, NOT applied now).** OptionsList.h: `v(Size, maxJSThreadHeapBytes, 0, Normal, ...)` adjacent to `maxJSThreads` — **pre-applied as an inert stub at charter time (this round)**. Heap.h (GCClient::Heap): `Atomic<size_t> m_grossBytesAllocated`, `size_t m_grossBytesAtLastCycleEnd`, `bool m_heapQuotaExceeded`. Heap.cpp: B.1 bump + B.3 check in the `didAllocate` ISS leg; B.3 snapshot/clear in the `updateAllocationLimits` forEach; `formatClientAttribution`. LocalAllocator.cpp / CompleteSubspace.cpp: B.3 sticky-bit early-out (ReturnNull-mode only) under MSPL. VMTraps.cpp `handleTraps` + per-lite trap fire (out-of-tree manifest hunk): B.3 quota-RangeError raise. runtime/ExceptionHelpers or ErrorInstance (out-of-tree manifest hunk): append B.2 string when ISS. Regression: `JSTests/threads/heap-quota-attribution.js` (§12.1 `allocationStorm` variant: one storming client + one quiescent client; assert (i) quota=0 ⇒ storming client OOMs at the whole-heap limit with attribution naming it top-1, quiescent client unaffected; (ii) quota=small ⇒ storming client throws RangeError with the quota clause, quiescent client allocates past that point).
