...
4. Destroying a VM that ran Threads (tenth landing round; history r10.2). Completion (item 1) is published before the native thread is gone: after it, the thread still runs its exit tail and then drops the `Ref<VM>` its entry closure holds. An embedder that joins every Thread and then releases its own reference can therefore find that reference is not the last one; the last one is then dropped by the exiting thread, outside the API lock, and `~VM` fail-stops there (MC-TDWN S1: `~VM` requires the API lock). The embedder contract is: join every Thread; call `ThreadManager::waitForCompletedThreadsToReleaseVM(vm)` WITHOUT holding the API lock (GIL on the exiting thread needs it for its tail); then release the VM under the API lock. The call returns when no native thread that has published completion for this VM still holds its closure's reference; it does not wait for a Thread that is still running (destroying a VM under a running Thread stays a contract violation and keeps the fail-stop). The shell's `--destroy-vm` and its worker teardown make the call. The count is kept per VM address in ThreadManager (incremented immediately before the Phase release-store at both publication sites, decremented at the end of the entry closure after the reference is gone), so nothing in it depends on the VM's lifetime.
...
3. Choke-point hook=9.2 entry 6,INTEGRATOR-applied after obj-model diff. Hook text:every generic-path entry point of Dev 8's enforced set - getOwnPropertySlotImpl (JSObject.h:1459),putInline*/putInlineSlow family (JSObjectInlines.h),putByIndex,deleteProperty+deletePropertyByIndex,defineOwnProperty,getOwnPropertyNames,setPrototype(Of),isExtensible,preventExtensions (JSObject.cpp);plus any successor generic entry point in the merged tree;MUST begin with
 ...
 Tenth round (hist r10.1): "SlowPut sticky => hooked generic paths" (item 1) had two exceptions in `main`'s code that store an EXISTING in-vector element of a SlowPut array without passing a hooked entry point - `JSObject::trySetIndexQuickly`'s SlowPut arm and DFG/FTL PutByVal under `Array::SlowPutArrayStorage` - which per-thread butterfly tags used to cover by accident (every store from a thread other than the allocator failed the write predicate). Whenever the flag is on the C++ arm returns false and the DFG treats an array profile that saw a SlowPut shape as `Array::Generic` (flag off: unchanged).
