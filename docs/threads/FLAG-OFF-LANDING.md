...
**State after the thirteenth round (2026-09-25).** The bullets below this block are the eleventh session's and are kept as
written; where a number here differs, this one is the later measurement. LANDING-PLAN "Results, twelfth and thirteenth rounds"
has the tables.
- *Speed.* Quiet pass 0.989 of `main` (Startup 0.986, Worst Case 0.983, Average 0.991), from 0.966. Main-thread cycles: first
  iteration 1.008 to 1.015 (from 1.039), interpreter only 1.011 per test (from 1.041), Baseline-capped 1.008 to 1.012,
  DFG-capped 1.007 to 1.016, whole run 1.013 (from 1.013: it did not move). Instructions are at 1.005 to 1.016 in every metric,
  about half of their excess before the round. Of the L5 targets the interpreter's is met within the noise, the score is one
  thousandth short, the others are not met.
- *How.* The process's threads mode is one byte that the compiler treats as constant; functions are compiled once per mode and
  the copy without threads has no gate in it (`runtime/ThreadsModePage.h`; SPEC-ungil history, thirteenth round). The
  interpreter has `main`'s bodies (twelfth round) and calls the slow-path copies of the process's mode through a table.
- *What is left of the gap* is attributed by exact instruction counts (LANDING-PLAN, "Not done"), and by what instruction counts
  do not see: the whole run's cycles did not follow its instructions.
- *Behaviour.* Unchanged: the stress suite fails the same 21 of 87,472 plans as `main`, test262 is identical on 102,475 records,
  the test executables agree, the golden compare differs from `main` by less than `main` differs from itself.
- *Platforms.* The C loop builds and runs (it did not build; `main` does not build in that configuration). arm64: the C++ passes
  the syntax check without an error that is the branch's, the interpreter is generated by the arm64 backend and assembles.
  Nothing was built or run on arm64 hardware, macOS, Windows or musl.
- *Size.* Text +19.1 % over `main` (+15.6 % before the round): the per-mode copies cost 1.09 MB.
- *The embedder.* Its patch is refitted to Bun's current `main` and was not built (the build needs a newer clang than the
  machine had).
...
**After the thirteenth round** (same session as its `main`; LANDING-PLAN has the before and after of the round, and which
harness each number is from):

| metric (flag off / `main`) | after the thirteenth round | target | met |
|---|---|---|---|
| first iteration, main-thread cycles | 1.008 (three rounds), 1.015 (five rounds) | 1.010 | not reliably |
| interpreter only, cycles, per-test geometric mean | 1.011 | 1.010 | within the noise |
| Baseline-capped / DFG-capped, cycles | 1.008 to 1.012 / 1.007 to 1.016 | 1.005 / 1.010 | no / not reliably |
| whole run, main-thread cycles | 1.013 | 1.005 | no |
| locked loads, whole run / first iteration | 0.96 / 1.02 | 1.02 / 1.03 | yes |
...
drain loop (the rest of L5.7) and L5.11 to L5.14 are not done; L5.5 and L5.8 were not re-measured.
