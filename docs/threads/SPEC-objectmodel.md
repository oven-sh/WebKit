...
Status: FROZEN rev 18 ("annex"=docs/threads/SPEC-objectmodel-annex.md, FROZEN NORMATIVE, MUST be consumed; "history"=SPEC-objectmodel-history.md, NON-NORMATIVE; r2-r4: history §19-§21). Master flag=`Options::useJSThreads()` (§9.6). Pinned: jit r12, api r14, heap r13, vmstate r13 (on-disk revs); cites: in-tree=file:line, cross-spec=section anchors. STW=stop-the-world, WS=watchpoint set, flag-on/off=useJSThreads, GT#n=§1 row n, B=flat butterfly ptr, AS=ArrayStorage/SlowPut shape, PA cell=isPreciseAllocation(cell) (PreciseAllocation.h:68-70; 8-mod-16; MarkedBlock cells 16B-aligned). NBR=annex sections (GT1-GT17; §14; §15.T12; App. R8; §16.8; §L; §L2; §M; §Q); spec wins on conflicts.
...
Deviations: FULL list=annex §14 (NBR)+r12: E4/F2 per-object keying (§5), shared ContiguousDouble (§4.7), O4 allocation-free stops (§6), manifest 7b, 8c rebias charter; r13: per-server-heap quarantine epoch (§6), F4 chain-fire (§5); r14: L6/I37 (§6), 8h survey, prep (§9.6); r15: N2-LF (§2.1), L4-K (§6), I38/I39, history §23; r16: N1-I instance-keyed butterfly-less ownership (§2, §2.1 N1/N2/N3, §5 E4/F2, I2/I40), F4 chain-fire withdrawn (§5), M8 GIL-on unforced, §4.6 AS-INPLACE, T4 Undecided owner relabel, history §24; r17: T4-O owner relabels of typed sources stop-free (§4.4 T4, §4.7, I28, I41), owner claim keyed on set validity (lock-free) else cell-locked, GIL-off Int32->Double executed as Int32->Contiguous, M8 in-place growth restored GIL-on, history §25; E4-C owner claim-first leg after the F2 fire, locked writers claim-first with RESTART on a lost claim (§4.3 step order, N2 (ii)), I38 restated, history §26; r18: T4-C fresh copies of Double sources are Contiguous GIL-off, T4-P optimized allocations report to and watch their allocation profile GIL-off, history §27.
...
Decode (total): payload==0=>None (§2.1) — r16: the tag of a None word names the INSTANCE OWNER: (t, SW=0, payload 0) = "butterfly-less, allocated by t"; the all-zero word is the t=0 case (main thread GIL-on / flag-off / cells built without a TID); (t, SW=1, 0) and (notTTLTID, *, 0) illegal. payload!=0, t!=notTTLTID: SW=0=>Flat (owner t transitions lock-free, §5), SW=1=>Flat shared-written (any transition=>regime 2); TID=notTTLTID, SW=1, payload!=0=>Segmented (spine payload); notTTLTID+SW=0 illegal (I3).

**G1 - one owner with the GIL (tenth round, history §39).** In a GIL-on process (`!gilOffProcess`; option `useJSThreadsSingleOwnerWithGIL`, default true) `currentButterflyTID()` is 0 on every thread, the JIT's per-thread tag word (jit R5) likewise, so every word is born with tag (0, 0), every thread passes every owner test, and nothing is ever foreign: no F1/F2 fire, no SW flip, no flat->segmented conversion, no thread-local set fires for a threading reason. Every butterfly word in the process is then its untagged payload, as flag off, and generated code may access it as flag off does (jit §5.5 "Untagged words"). Sound because GIL on the running mutator has no concurrent mutator, exactly E4-G's premise: the GIL is handed over only inside blocking primitives, which are calls, and no object-model window or piece of generated code holds a storage pointer across a call; the marker and the compiler threads are the concurrent parties flag off has. The stress options of §9.6 (`forceSegmentedButterflies`, `forceButterflySWBit`) still produce tagged words; with either set, or with the option off, the process counts as tagged (`Options::useTaggedButterflies()`, computed once the options are final: `useJSThreads && (!useThreadGIL || !useJSThreadsSingleOwnerWithGIL || forceSegmentedButterflies || forceButterflySWBit)`). In an untagged process `ensureSharedWriteBit` returns at once (the caller is an owner) and `convertToSegmentedButterfly` is unreachable (release assertion). The object model's C++ entry points (named and indexed puts, deletes, storage growth, indexing-shape changes, CopyOnWrite materialization, the Structure table operations of L6, the array fast paths' snapshot probes) key on the same predicate and take their flag-off bodies there: those are `main`'s, written for one mutator at a time, which is what the GIL provides, and an untagged word is the representation they expect; the flag-on bodies stay reachable from generated stubs that still call them and are equally valid on untagged words, so the two may mix. What stays keyed on `useJSThreads` in those files is what is not about concurrent mutators: the thread-restriction checks (api §5.8) and the assertions inside the concurrent helpers. Thread identity (`Thread.id`, lock ownership, the ThreadManager's TIDs) is unaffected: only the butterfly tag is shared.

Dispatch tests payload==0 BEFORE any TID compare; TID 0 (main)=>bit-identical to today. Flag-on, EVERY JSObjectWithButterfly is born with the word `encodeButterflyTag(currentButterflyTID(), 0) | payload` (payload 0 when butterfly-less): C++ constructors (incl. the well-defined-builtin-cell form) and every JIT inline allocation (jit §5.5 Allocation) stamp it pre-escape. api §5.1 allocates TIDs (over cap=>RangeError); we consume currentButterflyTID() (vmstate §6.7). No recycling this milestone (8c charter); tags sticky; 2^15 cap.

### 2.1 Regime None (first install / inline adds)

- N1 (r16: N1-I, instance-keyed). Butterfly-less ownership = the word's tag TID, exactly as butterfly-bearing (E4): the sole lock-free transitioner/first-installer of a None-word object while the source TTL sets are valid is the thread whose currentButterflyTID() equals butterflyTID(word); every other thread fires first (F2). `Structure::m_transitionThreadLocalTID` (r12-r15's shape key) no longer decides ownership anywhere (kept, inherited as before, informational). Why: shape keying made every spawned thread FOREIGN to its own fresh objects whenever the shape tree was first built by another thread (the common case: the empty-object structures are the main thread's), so the first add on any other thread fired the root shape's sets (F4: every later descendant born fired) and all threads then took the locked N2 for plain objects forever; measured 230x-400x on two threads of independent object creation. Instance keying makes thread-private objects owner-fast on every thread and fires only on a real cross-thread transition.
- N2. Structure-only transitions (butterfly untouched): (i) E4->today's code, or (ii) cell lock, CLAIM (CAS StructureID S->nuked(S); lost=>unlock, RESTART), then release-store the inline value (I9), then the header CAS from the nuked header under §3.0 (r17: claim before value, history §26), or (iii) N2-LF (r15, the form JIT caches emit; C++ may): pre E4's predicate holds by FRESH loads in a poll-free window (both source TTL sets valid+watched, !PA, payload==0, tag==(currentButterflyTID(),0) [r16; r15 keyed this on the structure TID], source not AS-shaped, target same outOfLineCapacity); then (1) 32-bit CAS m_structureID old->nuke(old) — failure (nuked or moved)=>abandon with NOTHING written, slow path re-dispatches; (2) release-store the inline value; (3) storeStoreFence; (4) store the new un-nuked StructureID. The claim in (1) precedes every store, so a loser never writes a slot another transition owns (the r14 8h objection to a value-first header CAS does not apply); the window (1)-(4) is straight-line, poll-free, allocation-free (I29). Interference: history §23. Existing-slot inline access stays lock-free.
- N3. First install: CAS the None word w (r16: w = (t,0,0), the value LOADED, not literal 0)->`encodeButterfly(b, currentButterflyTID(), 0)`; 64-bit CAS if header unchanged, else §4.3 DCAS. Failure=race lost: re-dispatch on the winner's tag. E4-eligible installs may plain-store as today. A foreign (locked / per-event-stop) first install stamps the INSTALLER's TID (the previous owner's lock-free rights ended with the F2 fire that preceded it).
- N4. butterflyRegime()=None for payload 0 (any tag); None payloads never dereferenced.
...
2. Acquire cell lock.
3. Re-read under the lock: header, tagged word, flat vectorLength/publicLength (T5, where it exists - see T5's note: not in a GIL-off process - grows VL in place: pointer/tag unchanged). structureID != planning-time source->RESTART; segmented->unlock, retry segmented; flat pointer OR vectorLength changed->recompute slices, C2 counts, aliased base/size vs current B; counts no longer fit the step-1 spine/fragments->unlock, goto 1; C1/C3 violation=logic error.
4. (r17: MOVED after the claim of step 5.) Trigger adds a property->release-store its value into the fragment slot (M2/I9). The slot may ALIAS the live flat butterfly (I7), which the owner's claim-first legs (E4-C, N2-LF) may be writing until they lose the lane, so the store happens only once step 5's CAS holds the lane - before the DCAS publish, keeping I9 (history §26 addendum 1b; before r17 the store preceded the claim and an owner add racing the conversion could be aliased away, I21).
5. Nuke+publish under §3.0 (PA cell: I36 fenced order): CAS structureID->nuke(old) (lost => RESTART, nothing written); step 4's store; 128-bit DCAS {nuked header, expected flat word}->{new un-nuked header, spine tagged (notTTLTID,1)}. Failure->§4.3 taxonomy (its goto-3=THIS step 3 incl. refit escape).
6. Release cell lock.
...
4/5 (r17 order, history §26). CLAIM: CAS StructureID S->nuked(S); a lost claim=>unlock, RESTART (a lock-free owner leg held the lane). Then release-store the new property's value (M2/I9: value before type) — into private fresh storage it may equally be stored before the claim — then publish {new header, new tagged butterfly} via 128-bit DCAS from the nuked header; PA cell: I36 fenced order.
6. Release cell lock.
...
- T4. Indexing-SHAPE transitions (e.g. Int32->Contiguous): full §4.3. Double: §4.7; AS: §4.6. r16 UNDECIDED-SOURCE OWNER RELABEL: Undecided->{Int32,Double,Contiguous} on a flat word the current thread owns ((currentTID,0), storage present) is claim-first and stop-free: CAS StructureID S->nuked(S) (excludes a concurrent transition or SW flip of this object, whose stop/DCAS re-verifies or carries the ID lane), re-check the word, write hole/PNaN lanes through vectorLength, storeStoreFence, publish S'. Sound because no tier loads an element lane from an Undecided-shaped object (readers holding S read nothing; readers loading S' see initialized lanes); r17 T4-O TYPED-SOURCE OWNER RELABEL (history §25), same owner predicate (flat SW=0 word (currentTID,0), storage present, not CoW; PA allowed): GIL-on (vm not gilOff) Int32->Double, Int32->Contiguous, Double->Contiguous rewrite lanes IN PLACE with the flag-off bodies, stop-free — sound because the phase-1 GIL is handed off only inside calls and no tier keeps a shape check, butterfly pointer or open fast-path window across a call (jit I21 GIL-on argument); GIL-off ONLY Int32->Contiguous (no lane rewrite) is stop-free, an Int32->Double request is EXECUTED AS Int32->Contiguous by every thread (the array never gains raw-double lanes after admitting JSValue-keyed readers; I41 lists what a stale Int32-keyed reader may then see and who tolerates it), Double->Contiguous keeps the per-event stop (a stale Double-keyed reader over boxed storage would box an impure NaN or read a pointer's bits as a double; readers are check-then-load with the check hoistable across polls GIL-off, so no publication order of a copy-convert excludes the (old shape, new storage) pair — history §25, §27). r18 removes the Double arrays that were only ever going to be converted instead: T4-C — GIL-off a FRESH array made by copying a Double source's lanes (`slice`, `concat`, the DFG `ArraySlice` intrinsic, which GIL-off neither plants on Double-typed sites nor admits Double arrays) is created Contiguous with the lanes boxed during the copy (new object, no reader, no hazard); T4-P — GIL-off DFG/FTL array allocations compiled from a profile recommending Double report their arrays to that profile's last-array word and watch a per-profile set the profile fires when its recommendation leaves Double, so the site recompiles to allocate what its arrays become (history §27). T4-P PROMOTION (rev 19, history §28; tenth round, history §40): GIL-off, when a site's recommendation moves from Undecided/Int32 to Contiguous, the site's demotion set is intact, and the array that moved it is a numeric one, the recommendation becomes Double instead, so the site's later arrays are born Double and never convert. "Numeric" is either of: the array holds nothing but numbers with at least one non-int32 among the sampled lanes (rev 19), or the array is remembered as the subject of a SUBSTITUTED request - T4-O executed an Int32->Double request on it as Int32->Contiguous (tenth round). The memory is a small per-process table of array addresses written by the substituting thread and compared by address only (never dereferenced; an entry that outlives its array can only cause one wrong promotion, which the demotion set undoes once and for all for that site); a collection's end clears it. CLAIM for every owner leg (Undecided included, r17 correction): both source TTL sets valid by fresh loads in the poll-free window => lock-free CAS StructureID S->nuked(S); else the SAME sequence under the cell lock (foreign locked writers assert their step-5 nuke CAS, §4.2/§4.3, and are excluded only by a prior set-firing stop or by the lock); then re-load the word and require it unchanged (else un-claim, stop path), rewrite, storeStoreFence, publish S' (un-nukes). Foreign / SW=1 / segmented relabels keep the per-event stop (F2, I28). The racer-settled early return in the relabel loop carries a load-load fence since publications land outside a stop.
- T5. **WITHDRAWN in a tagged GIL-off process** (the tree removed it in the first review round; text corrected in the eleventh session: a published flat butterfly's vectorLength is immutable GIL off, which is the invariant SPEC-jit history §39 relies on; every owner growth is T1's copy; the only in-place forms left are the two in `ensureLengthSlowConcurrent`'s `!vm.gilOff()` branch - the raise into size-class slack and the PreciseAllocation realloc, M8 - reachable only in a tagged GIL-on process, `--useJSThreadsSingleOwnerWithGIL=0`; DESIGN-PROPOSALS section D, D-AG-2, records why an in-place raise is not proposed again). Original text: In-place vectorLength-only growth (ensureLengthSlow in-place branch, GT10): CELL-LOCKED, owner-only: lock; re-check tagged word == (currentButterflyTID(),0), changed->unlock, T2; clear [oldVL,newVL); fenced setVectorLength; unlock (bounded, O2; spine-covered: §4.2-3 re-reads VL under the SAME lock — history §16.1). SW=1/foreign OOB growth=T2, never in-place (I21).
...
AS-COPY: anything a stale lock-free reader DECODES - the butterfly pointer, the IndexingHeader, indexBias, vectorLength, out-of-line property slots - is NEVER relaid-out in place flag-on. shift/unshift head moves (`Butterfly::shift/unshift`, indexBias) and any vectorLength change allocate a fresh AS butterfly under the cell lock (per O1's pre-lock DeferGC exemption) and casButterfly it; superseded storage never written again (stale readers frozen; conservative scan, I7). In-place element/publicLength/m_numValuesInVector stores in an installed AS stay legal under the lock (bounds=its now-immutable vectorLength). AS-INPLACE (r16, owner only): on a word the current thread owns ((currentTID,0), I40) shift/unshift MAY instead move ELEMENTS within the installed vector under the lock (gcSafeMemmove down for shift, up into spare vectorLength for unshift, cleared slots become holes): a stale lock-free reader of an SW=0 AS word then sees per slot an element that was in the array or a hole - the same contract as the owner's in-place flat shift (§3) - and nothing it decodes moves; locked accessors and the marker (which locks AS butterflies) are excluded by the lock; no allocation. The O(1) head move stays AS-COPY until AS reads by non-owners are owner-checked (jit §5.5 Read has no TID test). Sparse-map structural edits stay in place: runtime-only=>locked both sides; jit never fast-paths sparse.

### 4.7 Double arrays

ContiguousDouble payloads=raw doubles (GT#15; racing in-place relabel=type confusion). R-DOUBLE (r12): shared Double STAYS Double — §4.2 of a Double-shaped object aliases/builds RAW-double fragments (aligned 8B slots: tear-free, SAB semantics; holes=PNaN as today); no rebox, no boxing, no sharing-onset STW. Slot interpretation is shape-keyed: readers M7-order or re-check structureID; GC skips value-visiting Double fragments (§4.5-5). Shape changes TOUCHING Double on an SW=1/segmented object (Int32<->Double, Double->Contiguous)=per-event STW relabeling slots in place (no reader holds the old shape across a stop, I34/M7); Int32->Contiguous on shared objects: §4.3. Owner SW=0 flat words: T4-O (r17) — GIL-on in place stop-free, GIL-off Double stays behind the stop and Int32->Double becomes Int32->Contiguous; r18 T4-C/T4-P keep GIL-off programs from minting Double arrays that are converted on arrival (history §27).
...
- E4 (r12 per-object; r16 one leg). Owner transition may skip cell lock and (D)CAS - today's code incl. nuking - iff source's both sets valid+watched AND !isPreciseAllocation(cell) AND tag==(currentTID,0) — for butterfly-bearing AND butterfly-less words alike (N1-I; NO structure-TID compare anywhere — foreign-thread shape reuse stays lock-free). (I15/I36; sound only with I29.) E4 elides CELL-level sync only; Structure-table ops obey L6. r15: JIT tiers emit E4 for non-reallocating transitions (jit §5.5 Transition): butterfly-bearing per the tag compare (inline or existing out-of-line capacity; AS-shaped sources excluded), butterfly-less (payload 0) in the N2-LF form in the inline caches (GIL off; the CAS claim is kept as a tripwire there) and as a plain store in DFG/FTL PutStructure behind CheckTransitionOwner (jit §5.5); the C++ payload==0 leg uses N2-LF; reallocating (out-of-line growth) transitions were R3 until r17 and are now emitted too: claim-first with a word re-check in the inline caches, E4's plain order under the watched sets in DFG/FTL with an invalidation point between allocation and install (jit §5.5 (RE)ALLOCATING form). E4-C (r17): with the source's sets DEAD, the owner (same tag test, !PA, source not AS/CoW/dictionary) transitions CLAIM-FIRST lock-free instead of taking the lock: [allocate grown storage; re-read ID+word], CAS S->nuked(S) (lost=>RESTART), re-check word (moved=>un-claim, RESTART), store value (inline / existing out-of-line / private grown copy + casButterfly), storeStoreFence, store S'. Exclusion with foreign writers is the lane: every cell-locked writer claims before writing shared storage (§4.3 step 4/5, N2 (ii)) and RESTARTs on a lost claim; SW flips need the un-nuked header. JIT: the inline caches watch no TTL sets; in a GIL-off process (gilOffProcess) they emit the claimed form in both legs (jit §5.5), GIL on the unclaimed form E4-G (r19): owner predicate unchanged, then non-reallocating: value, storeStoreFence, S'; (re)allocating: nuked(S), storeStoreFence, tagged word, fence, S', with no word re-check; the C++ E4-C leg GIL on stores nuked(S) instead of claiming it, after its post-allocation re-read and inside its AssertNoGC window. Tenth round (history §37): GIL on the C++ publication helpers drop their atomic read-modify-writes too - `storeTaggedButterflyWordConcurrent` and the E4-C growth publication STORE the tagged word (the owner witness stays an assertion; the SW bit of the word read is carried over as before), the N2-LF butterfly-less leg stores nuked(S) instead of claiming it, and the bulk butterfly copies/zeroes (`butterflyConcurrentCopyWords`/`ZeroWords`) are the flag-off memcpy/memset: the only concurrent parties GIL on are the marker and compiler threads, which never write an object's word, StructureID or storage, and every publication keeps its fences and its nuke-word-ID order for them. A GIL-off process keeps every CAS and the word-wise copies. History §38: GIL on, the OWNER's §4.8 CopyOnWrite materialization (word tagged (currentTID, 0)) is E4's fenced publication as well - copy, storeStoreFence, nuked(S), fence, tagged word, fence, S' - with no cell lock, no re-verification and no DCAS; a materialization by a thread that is not the word's writer still fires F2 and takes the cell-locked route, in both GIL modes. DFG/FTL inline transitions keep E4's claim-free form under watched sets.
...
- F2 (r12; r16 one trigger). Fire BOTH sets on S and target, before cell-lock acquisition (I10b), on a transition of an S-instance by a thread != its tag TID or with SW=1 (segmented always) — butterfly-bearing and butterfly-less alike (N1-I). A tag owner transitioning its own instance through a foreign-created S does NOT fire.
...
- F4 chain-fire: WITHDRAWN in r16. r13 made F1/F2 stops also fire the still-valid sets along the fired structure's previousID chain and its whole transition-table subtree, purely to bound the number of stops during N-thread warm-up when shape keying made every thread foreign to everything. With N1-I fires happen only on genuine cross-thread transitions, and the chain turned one shared object's first foreign add into the retirement of lock-free/cached transitions for the entire shape family (every descendant of `{}`); r16 fires exactly what F1/F2 name (S; S and target). Soundness is per-structure and was never provided by the chain (it was additive). The F4 born-fired rule for NEW targets of a fired structure (Structure transition constructor) is unchanged.
...
- L4. Deletion and attribute reconfiguration. L4-K (r15, GIL-off): a defineOwnProperty that changes an EXISTING property of a cacheable (non-uncacheable-dictionary) structure between data and accessor/custom (the Accessor or CustomAccessorOrValue attribute bit differs) stores the new value and publishes the new StructureID inside ONE per-event STW (plan/allocate the target structure before the stop; inside: re-verify structureID and, for a cacheable dictionary, the pinned table + edit stamp, else RESTART; store; storeStoreFence; setStructure; the window's in-window heap-fact epoch bump is mandatory). Reason: the slot keeps its offset and changes KIND, and every tier's lock-free reader is a structure check followed by a slot load with no re-check, so a reader between the two would take a GetterSetter cell for a value under the data structure or call a value under the accessor structure; a stop leaves no reader inside such a window (IC/LLInt windows are poll-free; DFG code parked at a poll with a hoisted check is jettisoned by the bump). C++ readers that can park between offset and load (Structure::get may materialize) re-validate the raw structureID after the load and never hand out an accessor cell under data attributes (getOwnNonIndexPropertySlot, getDirect(VM&, PropertyName), fastGetOwnProperty) — I39. Uncacheable dictionaries keep the locked in-place edit (no tier caches them; C++ readers use the table's edit stamp). GIL-on/flag-off: unchanged (no reader can interleave).
- L5. ALL AS-object access flag-on, reads included (= I31).
- L6 (r14)=I37, shared-Structure tables. Flag-on: (i) mutator transition-table LOOKUPS use the m_lock-holding Concurrently variant (Structure.cpp:563->Structure.h:303; inserts already m_lock-held); (ii) PropertyTable steal/clone/materialize (takePropertyTableOrCloneIfPinned Structure.cpp:912-924, materialize) run under the SOURCE's m_lock — a stolen/fresh table is private until its new Structure publishes (mutate lock-free); every table mutation of a PUBLISHED Structure (addPropertyWithoutTransition, removes, dictionary=L3) holds ITS m_lock; (iii) mutator uncached table WALKS (StructureInlines.h get/find families) hold m_lock across the walk. SAL still wraps allocating inserts (7a<10b); compiler-thread Concurrently readers unchanged; flag-off=today (I22). L6-G (tenth round, history §36): in a GIL-on process (gilOffProcess == 0) rule (iii) does not apply - the mutator's uncached walk is the flag-off walk (materialize if needed, plain table probe): the writers (iii) excludes are other MUTATORS (dictionary edits, table steals, materialization), a mutator holds the VM's GIL for the whole of a C++ property operation and the GIL is handed over only inside blocking primitives (jit I21, E4-G's argument), so no such writer runs between the walk's loads; compiler threads and the marker never mutate tables and keep their Concurrently readers, exactly as flag off. Rules (i) and (ii) stay in both modes (they cost nothing measurable).
...
- M2. Step-4 value stores (§4.2/§4.3/N2, N2-LF step 2, L4-K in-window store): release (L4-K: plain in-window + storeStoreFence before setStructure).
...
- M7. Reader structure->butterfly ordering (arm64): a structure check+SEPARATE butterfly-word load must order the structureID load first (else stale-smaller-storage OOB, I24). Conforming: (a) address dependency (Dependency::consume, JSObject.cpp:385,400); (b) load-acquire structureID; (c) re-load/re-compare after (JSObject.cpp:403-405; mandatory on nuke-tolerant paths); (d) loadLoadFence just before the tagged-word load. x86-64: no-ops. JIT via E3; owned runtime: (a)/(c); locationForOffset/§9.5: (d). GAP (eleventh-session reading pass, DESIGN-PROPOSALS N2/N4): M7 as written covers the butterfly-word load only. An INLINE (in-cell) slot read that a structure check licenses has no butterfly load in between and nothing orders it after the structure-ID load on arm64 in the handler inline caches (`loadHandlerImpl` -> `CCallHelpers::loadProperty`, inline leg) and in DFG/FTL `CheckStructure` + `GetByOffset`; the LLInt threaded forms, the Baseline packed self word and the megamorphic probe are covered by construction. A reader can then see the new structure ID and the slot's zero fill. Two butterfly-word gaps also remain (`loadButterflyWithStructureDependency` when dest == base; the DFG Spread/ArraySort/enumerator choke calls). x86-64 is unaffected. Proposed closure: every guard load of the structure ID or indexing byte is a load-acquire on weakly ordered targets GIL off, in every tier (N-D2), which subsumes (a)-(d) for generated code.
- M8. Flag-on, the fenced nuke/publication order is the only branch and in-place butterfly reallocs are disabled. r16: this no longer forces `m_mutatorShouldBeFenced` GIL-on (its cost was every write barrier through the slow path with a store-load fence): the flag-on C++ publication sites fence unconditionally (`nukeStructureAndSetButterflyConcurrent`, `isX86() ||` guards), in-place realloc of a PreciseAllocation-backed butterfly is again allowed GIL-on (r17: gated on `!vm.gilOff()` plus the stock `!mutatorShouldBeFenced`, which a shared collector forces, so GIL-off stays excluded — a stale foreign reader may be loading from the block the realloc frees; history §25), and GIL-on another mutator runs only across a GIL hand-off (a full barrier), so the JIT publication sequences need the runtime fence only against a concurrent marker (the stock marking-time toggle). GIL-off keeps the heap-lifetime forcing (`Heap::setMutatorShouldBeFenced` pins it once the server is shared; JIT code reads the server flag) until the per-client barrier-flag reroute. (Corrected in the eleventh session: that sentence describes x86-64. `VM::VM` forces the fenced barrier whenever `useJSThreads && !isX86()` and `Heap::setMutatorShouldBeFenced` keeps it raised for `!isX86()`, so on every non-x86 target BOTH flag-on modes run with every write barrier on its slow path for the heap's life; the comment above the forcing in `VM.cpp` says GIL on no longer forces it. DESIGN-PROPOSALS N6 / N-D4 proposes keying the publication fences on the GIL-off process and the barrier threshold on marking.)
...
- I2. `tagged & ~butterflyTagMask`=null or a valid live butterfly/spine; null payload=>SW=0 and TID!=notTTLTID (r16; the TID is the allocating thread's).
...
- I9. A reader seeing a transition's new StructureID sees the new property's value (release-stored first), modulo M7. (On arm64 this does not hold today for inline slots read by handler ICs and DFG/FTL code: see the GAP note in M7.)
...
- I21. No lost property adds (incl. N2/N3 races), torn JSValues, structure/butterfly mismatch, or read of f returning g's value, under any race. In every tier: generated length updates follow SPEC-jit §5.5's length-update rule (ninth round, history §34).
...
- I28. Shape changes touching Double on SW=1/segmented objects happen only under per-event STW; no reader interprets a slot under a shape not M7-ordered/re-checked vs the loaded spine (§4.7). r17: GIL-off, NO stop-free relabel changes lane representation (Int32->Contiguous only; Int32->Double executed as Int32->Contiguous); GIL-on the owner's in-place rewrites are excluded from every other mutator by the GIL hand-off discipline (T4-O).
...
- I42 (ninth round; history §30). GIL off, a consumer that reads several slots of one object under a single structure validation and stores what it read (the fast data-property copies, the clone arms) re-validates after its reads - load-load fence, the same structureID, no empty value among the values - and otherwise takes the generic per-property path; it writes nothing before that check. A consumer whose stores go to a fresh object no other thread can reach (`Object.values`' result array, tenth round; history §35) may store before the check, provided every walk uses the one Structure snapshot, dictionaries (tables that change in place) are refused, every store is bounded by the count the object was allocated for, and the fresh object is dropped when the check fails. The rule covers every reader that takes a slot by an offset it listed from a structure, whether or not it stores the value (tenth round; history §41: the JSON stringifiers, which hand the value to a walker that dereferences it, and the `Object.entries` and `Object.defineProperties` fast paths): a cacheable dictionary's in-place add lists the property before its value is stored, under the writer's cell lock, which these readers do not take; a reader that interleaves reads with calls checks each value as it reads it. The window itself was a writer-side defect and is closed there (tenth round; history §41, amendment): a dictionary's in-place add stores the value BEFORE the table entry is published, as every other flag-on add does (I9), so a name never resolves to a slot that still holds EMPTY or a previous occupant's residue; the after-read check stays as the guard against a transition or a delete that races the reads.
...
- I38 (r15; r17 restated). A claim-first transition (N2-LF, E4-C, the claimed IC/megamorphic stubs, the T4-O relabel) writes NOTHING another claimant can reach before its StructureID claim succeeds and publishes the new ID only after the value (M2) and a storeStoreFence; every other writer of that object's StructureID lane is the same thread, or claims the lane first itself (every cell-locked writer, r17) and RESTARTs on a lost claim, or is a claim-free owner window that exists only while the source's sets are valid (when every foreign writer stops the world first), or runs world-stopped. GIL on (gilOffProcess == 0; r19, E4-G) an owner window may also skip the claim, because no writer's window contains a GIL release point: the GIL is handed over only inside blocking primitives (jit I21), locked protocols never park (O2), the inline-cache and megamorphic windows are straight-line code, and the C++ legs are AssertNoGC from their re-read to their publish; so no other mutator can be inside a window on the same object. Hence no two writers hold the lane at once; a lost claim under the cell lock is a RESTART, any OTHER semantic divergence there stays taxonomy (d). (r18 note: this includes the structure-only legs that publish with one header CAS and no nuke of their own - delete, attribute change: a header CAS that fails on the ID bits IS the lost claim there, history §27 follow-up F33.)
- I40 (r16). Flag-on, every JSObjectWithButterfly leaves its constructor / JIT allocation sequence with butterflyTID(word)==currentButterflyTID() of the allocating thread and SW=0, before the object can escape; no path stores an all-zero word over a tagged None word except the dead-TID restamp (which runs world-stopped and fires nothing: a None word has no structure sets to retire beyond F2's, and TID 0 owns nothing GIL-off).
- I41 (r17). GIL-off, a reader keyed on Int32 shape may load a NON-Int32 JSValue from a lane (a stale check across an owner's stop-free Int32->Contiguous), never a non-JSValue; every consumer tolerates it: DFG/FTL Int32-mode `GetByVal`/`ArrayPop` verify isInt32 and exit; C++ lane readers test isInt32 before asInt32 and fall to their generic path; C++/DFG COPY paths never memcpy lanes of a FOREIGN-owned Int32-shaped source into an Int32-labelled result (checked copy demoting to Contiguous, or generic path; owned sources memcpy as before — only the owner relabels stop-free); the marker value-visits Int32-shaped lanes GIL-off (defense for a missed copy site: a mislabel, never an unmarked cell). GIL-on/flag-off: unchanged.
- I39 (r15). GIL-off, no reader returns an accessor cell (GetterSetter/CustomGetterSetter) as a data value or calls a data value as an accessor: kind changes of an existing slot are L4-K stops; C++ readers whose offset->load window can park re-validate the raw structureID (and pinned-table stamp) after the load.
