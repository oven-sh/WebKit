...
Status: FROZEN rev 16 ("annex"=docs/threads/SPEC-objectmodel-annex.md, FROZEN NORMATIVE, MUST be consumed; "history"=SPEC-objectmodel-history.md, NON-NORMATIVE; r2-r4: history §19-§21). Master flag=`Options::useJSThreads()` (§9.6). Pinned: jit r12, api r14, heap r13, vmstate r13 (on-disk revs); cites: in-tree=file:line, cross-spec=section anchors. STW=stop-the-world, WS=watchpoint set, flag-on/off=useJSThreads, GT#n=§1 row n, B=flat butterfly ptr, AS=ArrayStorage/SlowPut shape, PA cell=isPreciseAllocation(cell) (PreciseAllocation.h:68-70; 8-mod-16; MarkedBlock cells 16B-aligned). NBR=annex sections (GT1-GT17; §14; §15.T12; App. R8; §16.8; §L; §L2; §M; §Q); spec wins on conflicts.
...
- N2. Structure-only transitions (butterfly untouched): (i) E4->today's code, or (ii) cell lock+header CAS under §3.0, release-storing the inline value first (no holes, I9), or (iii) N2-LF (r15, the form JIT caches emit; C++ may): pre E4's predicate holds by FRESH loads in a poll-free window (both source TTL sets valid+watched, !PA, payload==0, tag==(currentButterflyTID(),0) [r16; r15 keyed this on the structure TID], source not AS-shaped, target same outOfLineCapacity); then (1) 32-bit CAS m_structureID old->nuke(old) — failure (nuked or moved)=>abandon with NOTHING written, slow path re-dispatches; (2) release-store the inline value; (3) storeStoreFence; (4) store the new un-nuked StructureID. The claim in (1) precedes every store, so a loser never writes a slot another transition owns (the r14 8h objection to a value-first header CAS does not apply); the window (1)-(4) is straight-line, poll-free, allocation-free (I29). Interference: history §23. Existing-slot inline access stays lock-free.
...
- T4. Indexing-SHAPE transitions (e.g. Int32->Contiguous): full §4.3. Double: §4.7; AS: §4.6. r16 UNDECIDED-SOURCE OWNER RELABEL: Undecided->{Int32,Double,Contiguous} on a flat word the current thread owns ((currentTID,0), storage present) is claim-first and stop-free: CAS StructureID S->nuked(S) (excludes a concurrent transition or SW flip of this object, whose stop/DCAS re-verifies or carries the ID lane), re-check the word, write hole/PNaN lanes through vectorLength, storeStoreFence, publish S'. Sound because no tier loads an element lane from an Undecided-shaped object (readers holding S read nothing; readers loading S' see initialized lanes); every other source shape keeps the per-event stop (a lock-free foreign reader may be decoding the lanes being rewritten - the withdrawn "thread-local gate", JSObject.cpp). The racer-settled early return in the relabel loop carries a load-load fence since publications can now land outside a stop.
...
- I38 (r15). An N2-LF transition writes NOTHING before its StructureID claim succeeds and publishes the new ID only after the value (M2) and a storeStoreFence; every other writer of a butterfly-less object's StructureID lane either is the same thread, or holds the cell lock AFTER F2 fired the source's TTL sets in a stop (which retired every N2-LF emitter for that source: jit §5.3), or runs world-stopped; hence no two writers own the lane at once and a locked writer's §3.0 CAS never observes an N2-LF nuke (else taxonomy (d)).
