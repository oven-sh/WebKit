...
{
    add(hasher, __bit_cast<uint64_t>(number));
}

inline void add(Hasher& hasher, float number)
{
    add(hasher, __bit_cast<uint32_t>(number));
}

template<typename T> inline void add(Hasher& hasher, T* ptr)
{
    add(hasher, __bit_cast<uintptr_t>(ptr));
}
...
inline void add(Hasher& hasher, const AtomString& string)
{
    ...
    add(hasher, __bit_cast<uintptr_t>(string.impl()));
}
