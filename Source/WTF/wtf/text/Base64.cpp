...
template<typename CharacterType> static void base64EncodeInternal(std::span<const uint8_t> inputDataBuffer, std::span<CharacterType> destinationDataBuffer, OptionSet<Base64EncodeOption> options)
{
    ...
    if constexpr (sizeof(CharacterType) == 1) {
        size_t bytesWritten = simdutf::binary_to_base64(__bit_cast<const char*>(inputDataBuffer.data()), inputDataBuffer.size(), __bit_cast<char*>(destinationDataBuffer.data()), toSIMDUTFOptions(options));
...
static std::tuple<FromBase64ShouldThrowError, size_t, size_t> fromBase64Impl(std::span<const CharacterType> span, std::span<uint8_t> output, Alphabet alphabet, LastChunkHandling lastChunkHandling)
{
    ...
    auto result = simdutf::base64_to_binary_safe(__bit_cast<const UTFType*>(span.data()), span.size(), __bit_cast<char*>(output.data()), outputLength, options);
    switch (result.error) {
...
size_t maxLengthFromBase64(StringView string)
{
    ...
    if (string.is8Bit())
        return simdutf::maximal_binary_length_from_base64(__bit_cast<const char*>(string.span8().data()), length);
    return simdutf::maximal_binary_length_from_base64(__bit_cast<const char16_t*>(string.span16().data()), length);
}
