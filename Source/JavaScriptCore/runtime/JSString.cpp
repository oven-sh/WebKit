...
void JSString::dumpToStream(const JSCell* cell, PrintStream& out)
{
    ...
    if (pointer & isRopeInPointer) {
        ...
        if (WTF::StringImpl* ourImpl = __bit_cast<StringImpl*>(pointer)) {
            if (ourImpl->is8Bit())
...
size_t JSString::estimatedSize(JSCell* cell, VM& vm)
{
    ...
    return Base::estimatedSize(cell, vm) + __bit_cast<StringImpl*>(pointer)->costDuringGC();
}
...
void JSString::visitChildrenImpl(JSCell* cell, Visitor& visitor)
{
    ...
    if (pointer & isRopeInPointer) {
        ...
        for (unsigned index = 0; index < JSRopeString::s_maxInternalRopeLength; ++index) {
            ...
            switch (index) {
            case 0:
                fiber = __bit_cast<JSString*>(pointer & JSRopeString::stringMask);
                break;
            case 1:
    ...
    if (StringImpl* impl = __bit_cast<StringImpl*>(pointer))
