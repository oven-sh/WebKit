...
private:
    ...
    public:
        ...
        // A port of V8's DateCache::DaylightSavingsOffset(). V8 takes two instants at most this far apart with the
        // same offset to have that offset in between, which tzdata contradicts (America/Recife was on DST from
        // 2000-10-08 to 10-15 only). Here a lookup more than this far past a cached interval starts an interval of
        // its own. Closer than this, the zone's transition table decides whether the lookup is in the interval's run
        // of one offset, which costs more than computing one offset but settles the run up to its next change.
