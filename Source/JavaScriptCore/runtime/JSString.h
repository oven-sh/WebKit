...
ALWAYS_INLINE void JSString::swapToAtomString(VM& vm, RefPtr<AtomStringImpl>&& atom) const
{
    ...
    markAsAtom();
    vm.heap.appendPossiblyAccessedStringFromConcurrentThreadsOrGCOwnedDataScope(this, WTF::move(target), gilOff);
}
...
ALWAYS_INLINE Identifier JSString::toIdentifier(JSGlobalObject* globalObject) const
{
    ...
    VM& vm = getVM(globalObject);
    SUPPRESS_UNCOUNTED_LOCAL StringImpl* impl = std::bit_cast<StringImpl*>(fiberBits);
    if (impl->isAtom()) {
        markAsAtom(); // Record for profiling.
    ...
    if (vm.gilOff()) [[unlikely]] {
        // Note: AtomStringImpl::add can atomize `impl` in place (flag read
        // is the atomic hashAndFlags), and if another thread already swapped
        // the cell to an atom, our extra swapToAtomString is an idempotent
        // atom-for-atom exchange.
