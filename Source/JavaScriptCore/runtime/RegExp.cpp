...
void RegExp::compileMatchOnlyHoldingCellLock(const AbstractLocker&, VM* vm, Yarr::CharSize charSize, std::optional<StringView> sampleString)
{
    ...
    // final value with a relaxed store (single byte, advisory).
    Yarr::ErrorCode constructionErrorCode = WTF::atomicLoad(&m_constructionErrorCode, std::memory_order_relaxed);
    Yarr::YarrPattern pattern(m_patternString, m_flags, constructionErrorCode, Yarr::ExecutionMode::MatchOnly);
    WTF::atomicStore(&m_constructionErrorCode, constructionErrorCode, std::memory_order_relaxed);
    if (hasError(constructionErrorCode)) {
        m_state = ParseError;
        return;
    ...
    if (!(vm->gilOff() && m_regExpBytecode)) [[likely]] {
        ...
        Yarr::YarrPattern bytecodePattern(m_patternString, m_flags, constructionErrorCode, Yarr::ExecutionMode::IncludeSubpatterns);
        if (hasError(constructionErrorCode)) {
            WTF::atomicStore(&m_constructionErrorCode, constructionErrorCode, std::memory_order_relaxed);
            m_state = ParseError;
            return;
        }
        m_regExpBytecode = byteCodeCompilePattern(vm, bytecodePattern, constructionErrorCode);
        WTF::atomicStore(&m_constructionErrorCode, constructionErrorCode, std::memory_order_relaxed);
        if (!m_regExpBytecode) {
            m_state = ParseError;
            return;
