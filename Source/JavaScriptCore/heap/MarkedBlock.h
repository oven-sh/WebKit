...
public:
    ...
    public:
        ...
        size_t backingStorageSize() { return __bit_cast<uintptr_t>(end()) - __bit_cast<uintptr_t>(pageStart()); }
...
public:
    ...
    {
        *__bit_cast<volatile uint8_t*>(&header());
    }
...
{
    return *__bit_cast<MarkedBlock::Header*>(atoms() + headerAtom);
}
...
{
    return __bit_cast<T>(header().m_verifierMemo);
}
