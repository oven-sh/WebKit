...
#if USE(BUN_JSC_ADDITIONS)
            if (isStartupSnapshotCell(executable)) [[unlikely]]
                return; // never written; whatever it gained after the restore was in the side table cleared above
#endif
...
void Heap::evacuateTablesForStartupSnapshot()
{
    ...
    rehomeOutOfSnapshotPages(m_weakGCHashTables);
    {
        ...
        decltype(m_snapshotUnlinkedCodeBlocks) fresh;
        m_snapshotUnlinkedCodeBlocks.swap(fresh); // empty at snapshot time anyway
    }
...
void Heap::freezeCurrentHeapAsImmortalStartupSnapshot()
{
    ...
    }
    {
        ...
        dataLogLnIf(Options::verboseStartupSnapshotFreeze(), "[imm] made ", count, " string cells' StringImpls static, and ", atoms, " atom-table entries (overlapping sets)");
    }
}
#endif
...
void Heap::addCoreConstraints()
{
    ...
    m_constraintSet->add(
        ...
        MAKE_MARKING_CONSTRAINT_EXECUTOR_PAIR(([this] (auto& visitor) {
            if (!m_objectSpace.hasImmortalBlocks())
                return;
            Vector<UnlinkedFunctionCodeBlock*> redecoded;
            {
                Locker locker { m_snapshotRememberedLock };
                redecoded = copyToVector(m_snapshotUnlinkedCodeBlocks.values());
            }
            for (auto* codeBlock : redecoded)
                visitor.appendUnbarriered(JSValue(static_cast<JSCell*>(codeBlock)));
