...
public:
    ...
    void appendPossiblyAccessedStringFromConcurrentThreadsOrGCOwnedDataScope(const JSString* owner, String&& string, bool gilOff)
    {
        // GIL-off, N mutators reach this from JSString::swapToAtomString
        // concurrently. An unlocked append races the vector's growth. GIL-on
        // and flag-off have one mutator (the JSLock holder) and the GC-end
        // prune runs world-stopped, so they keep the plain append.
        if (gilOff) [[unlikely]] {
...
private:
    ...
    // append; the GC-end prune runs world-stopped). NOT WTF_GUARDED_BY_LOCK:
