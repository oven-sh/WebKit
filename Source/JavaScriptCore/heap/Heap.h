...
public:
    ...
    void clearSnapshotUnlinkedCodeBlocks()
    {
...
private:
    ...
    UncheckedKeyHashMap<std::pair<const UnlinkedFunctionExecutable*, unsigned>, UnlinkedFunctionCodeBlock*> m_snapshotUnlinkedCodeBlocks WTF_GUARDED_BY_LOCK(m_snapshotRememberedLock); // visited as roots by the Img constraint
    UncheckedKeyHashSet<JSCell*> m_snapshotRememberedThisCycle WTF_GUARDED_BY_LOCK(m_snapshotRememberedLock);
    Vector<JSCell*> m_snapshotPreciseRoots; // filled once at freeze with the world stopped, read-only afterwards: no lock
#endif
