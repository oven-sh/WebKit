...
JSC_DEFINE_HOST_FUNCTION(functionAddressOf, (JSGlobalObject*, CallFrame* callFrame))
{
    ...
    if (!value.isCell())
        return JSValue::encode(jsUndefined());
    // Need to cast to uint64_t so __bit_cast will play along.
    uint64_t asNumber = __bit_cast<uintptr_t>(value.asCell());
    EncodedJSValue returnValue = JSValue::encode(jsNumber(__bit_cast<double>(asNumber)));
    return returnValue;
}
