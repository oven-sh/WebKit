...
// The startup deferral scale applies to the short wait too.
if (expectTierUp && typeof $vm.setStartupJITDeferralScale === "function") {
    $vm.setStartupJITDeferralScale(10);
    function deferredGet(o) {
        const tag = o.tag;
        return $vm.llintTrue();
    }
    const objects = makeObjects(16);
    const call = firstCallOutOfLLInt(deferredGet, i => objects[i % objects.length]);
    $vm.setStartupJITDeferralScale(1);
    if (!call)
        throw new Error("deferredGet did not leave the LLInt in " + calls + " calls");
    // 10 times thresholdForJITSoon (30) is 300 counts, 20 calls.
    if (call < missCount + 10)
        throw new Error("deferredGet left the LLInt in call " + call + ": the deferral scale did not apply");
}
