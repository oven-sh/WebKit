...
// The owner slot next to the embedder async-context slot ($vm.asyncContextOwner, a number)
// is captured with the async context when a promise reaction / async continuation is
// scheduled, and both are current while it runs. A job that captured neither runs with
// neither, and what a job installs does not leak into the next one.
...
const setOwner = v => $vm.setAsyncContextOwner(v);
