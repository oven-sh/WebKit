...
// too.
//
// The parser first reads "( ... )" as an expression. To see that it is a parameter list it parses it again, and that
// parse used to let `await` be an identifier in an async function. An arrow function in there went to the source provider
// cache that way, and the parse that knows about the async function skipped it from the cache. So the code below that
...
// [text before the expression, text after it]: code where `await` is not an identifier. The messages below are those of the
// first one.
const asyncFunctions = [
    ...
    // A class static block reserves `await` too.
    ["(class { static { (", "); } })"],
];
...
// In the body of an arrow function in a static block, `await` is an identifier again.
for (const [expression, , neverValid] of invalid) {
    if (!neverValid)
        shouldParse(`(class { static { () => { (${expression}); }; } })`);
}
...
// In module code `await` is reserved everywhere, and an AwaitExpression is valid at the top level. An arrow function there
// has the same parameters as one in an async function. The text before it is an expression that uses `await` the way the
// module allows it.
function shouldNotParseAsModule(source) {
    let error;
    try {
    ...
    if (error === undefined)
        throw new Error(`${source}: expected a SyntaxError in module code`);
}

function shouldParseAsModule(source) {
    try {
        checkModuleSyntax(source);
    } catch (e) {
        throw new Error(`${source}: ${String(e)}`);
...
for (const expression of [
    ...
    shouldNotParseAsModule(`export const x = await 1; (${expression});`);
    shouldNotParseAsModule(`(async () => { (${expression}); });`);
}
...
for (const expression of [
    ...
    shouldParseAsModule(`export let a, b, x = await 1; (${expression});`);
}
...
// The arrow functions do what they say, also the ones that the parser had to parse again for the async function.
{
    ...
    // A default value captures a variable that the body declares again.
    async function redeclared() {
        let N = 40;
        let f;
        [f = (b = N) => { var N = 7; return b + N; }] = [];
        return f;
    }
    shouldBe(settled(redeclared(), "redeclared")(), 47, "captured variable that the body declares again");
}
