...
function main() {
    ...
    const invalidDescriptors = [
        ...
        [{ args: ["i32"], returns: "napi_env" }, "napi_env as return type"],
    ...
    check($vm.ffiSignatureString({ args: ["napi_env", "napi_value"], returns: "napi_value" }), "napi_value(napi_env,napi_value)", "napi types");
    ...
    const everyAlias = [
        ...
        ["ptr", "ptr"], ["cstring", "cstring"], ["napi_value", "napi_value"],
    ];
    for (const [alias, canonical] of everyAlias)
        check($vm.ffiSignatureString({ args: [alias], returns: "i32" }), "i32(" + canonical + ")", "alias " + alias);
    // Every numeric tag in order.
    const canonicalByTag = ["char", "i8", "u8", "i16", "u16", "i32", "u32", "i64", "u64", "f64", "f32", "bool", "ptr", "void", "cstring", "i64_fast", "u64_fast", "function", "napi_env", "napi_value", "buffer"];
    for (let tag = 0; tag < canonicalByTag.length; ++tag) {
        if (canonicalByTag[tag] === "void")
            continue; // void is not a valid argument
