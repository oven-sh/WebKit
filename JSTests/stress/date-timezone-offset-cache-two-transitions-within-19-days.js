...
// DateCache::DSTCache (runtime/JSDateMath.cpp) caches the local time offset as intervals. To find the instant where
// the offset changes it bisects between two cached intervals at most 19 days apart. A zone can change its offset
// more than once within 19 days (America/Cambridge_Bay 2000-10-29 and 2000-11-05, America/Asuncion 2024-10-06 and
// 2024-10-15). Then a bisection probe can return an offset that matches neither interval. That probe has to start an
// interval of its own. It used to be filed under the later interval, so the instants from the probe up to that
// interval got its offset or its DST flag, and debug builds failed ASSERT(m_after->offset == offset).
//
...
function makeScenario(timeZone, firstTransition, lastTransition, checkNames)
{
    ...
    for (let time = sweep[0] - step; time <= sweep[sweep.length - 1] + step; time += step)
    ...
    // probe window past the first one holds both transitions, and the bisection towards the second one comes at the
    // three offsets from either side.
    const primings = [];
    ...
    return { timeZone, sweep, offsetAt, nameAt, offsetOracle, primings };
}
...
for (const scenario of scenarios) {
    // One pass over the window in order and one shuffled, on a fresh cache each, without any priming.
