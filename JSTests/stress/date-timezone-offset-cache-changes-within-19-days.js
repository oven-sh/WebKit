...
// follow the tzdata in use. A scenario whose transitions the tzdata does not have fails, unless they are predictions.
...
function makeScenario(timeZone, firstTransition, lastTransition, checkNames, predicted = false)
{
    ...
    const changesAt = (time) => offsetOracle(time - msPerMinute) !== offsetOracle(time) || longName(time - msPerMinute) !== longName(time);
    if (!changesAt(first) || !changesAt(last)) {
        if (!predicted)
            fail(`${timeZone}: the tzdata in use has no transitions at ${firstTransition} and ${lastTransition}`);
        return null;
    }
