...
// NonISODateUntil (CalendarICUBridge.cpp) counts the months of a lunar-month calendar (chinese,
// dangi, hebrew, islamic-*) from the epoch-day span and confirms the count with one ICU month add,
// instead of probing one candidate month at a time. Everything below compares a result either with
// the same difference computed another way or with a value that does not depend on ICU's data.
...
for (const calendar of ["chinese", "dangi"]) {
    ...
    const shortSpans = [
        ...
        ["2023-02-20", "2033-12-22"],
        ["1970-01-31", "1970-03-01"],
        ["2024-02-10", "2043-11-30"],
    ];
    ...
    checkDifference(day30, day30.add({ days: 150 * 29 + 17 }), `${calendar} day 30`);
    checkDifference(day30, Temporal.PlainDate.from({ calendar, year: 2031, monthCode: "M09", day: 29 }), `${calendar} day 30 to 29`);
}
...
// Adar..Elul of a common year when the sum lands in Tishri..Adar I; the bridge routes around that.
{
    ...
    checkDifference(Temporal.PlainDate.from("1880-08-06").withCalendar("hebrew"), Temporal.PlainDate.from("2943-11-02").withCalendar("hebrew"), "hebrew");
...
// islamic-*: twelve months a year, so whole years are a closed form to check against.
for (const calendar of ["islamic-civil", "islamic-tbla", "islamic-umalqura"]) {
    ...
    checkDifference(Temporal.PlainDate.from("0700-03-30").withCalendar(calendar), Temporal.PlainDate.from("2077-07-07").withCalendar(calendar), calendar);
}
