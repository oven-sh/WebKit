...
int main(int argc, char** argv) {
...

  {
    ...
    // icu/pin-strings-<icu major>.txt, i.e. it must not materialize a single
    ...
    if (bun_icu_fc_materialized_blocks != 0) {
      printf("FAIL the startup / first-Intl path materialized %d front-coded blocks;\n"
             "     icu/pin-strings-<icu major>.txt no longer covers it (regenerate the trace)\n",
             (int)bun_icu_fc_materialized_blocks);
      return 1;
    }
    // The pin trace for this phase is regenerated by running exactly this
    // much (see icu/trace-pool-strings.patch); everything after it is the
    // wide sweep that must NOT be pinned.
    if (getenv("BUN_ICU_TEST_STARTUP_ONLY")) return 0;
  }
