#!/usr/bin/env bash
# Run 3.6 A/B: d8ed7b6f5254 baseline (jsc-v33-baseline, sha 2a85f8e5…) vs v36
# (campaign-5/M1 BigInt + allocation/IC delta), same host, back-to-back.
set -u
SB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE="$SB/../../../WebKitBuild/Release/bin/jsc-v33-baseline"
V36="$SB/../../../WebKitBuild/Release/bin/jsc-v36"
...
RAW="$SB/results-v36ab-raw.jsonl"
ulimit -c 0
...
cell() {
    ...
    for rep in $(seq 1 $reps); do run1 "$label-v36" "$V36" "$W" "$envv" "$rep"; done
}
...
case "${1:-all}" in
    ...
    congc)
        echo "=== congc W=4 v36 only (loadavg $(awk '{print $1}' /proc/loadavg)) ===" >&2
        for rep in 1 2 3 4 5; do run1 "congc-w4-v36" "$V36" 4 "$PINNED JSC_useConcurrentSharedGCMarking=1" "$rep"; done
        ;;
    stab32)
        echo "=== W=32 stability v36 30 reps (loadavg $(awk '{print $1}' /proc/loadavg)) ===" >&2
        for rep in $(seq 1 30); do run1 "stab32-v36" "$V36" 32 "$PINNED" "$rep"; done
        ;;
esac
