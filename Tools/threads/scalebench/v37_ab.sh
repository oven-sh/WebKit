#!/usr/bin/env bash
# Run 3.7 A/B: d8ed7b6f5254 baseline (jsc-v33-baseline, sha 2a85f8e5…) vs v37
# (campaign-7 W>=2-only: T1-sibint distinct-allocating-clients eden-full gate;
#  T4 sibling-marking-assist admission cap; safepoint/park + MarkedBlock delta).
set -u
SB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE="$SB/../../../WebKitBuild/Release/bin/jsc-v33-baseline"
V37="$SB/../../../WebKitBuild/Release/bin/jsc-v37"
...
RAW="$SB/results-v37ab-raw.jsonl"
ulimit -c 0
...
cell() {
    ...
    for rep in $(seq 1 $reps); do run1 "$label-v37" "$V37" "$W" "$envv" "$rep"; done
}
...
case "${1:-all}" in
    ...
    congc)
        echo "=== congc W=4 v37 only (loadavg $(awk '{print $1}' /proc/loadavg)) ===" >&2
        for rep in 1 2 3 4 5; do run1 "congc-w4-v37" "$V37" 4 "$PINNED JSC_useConcurrentSharedGCMarking=1" "$rep"; done
        ;;
    stab32)
        echo "=== W=32 stability v37 30 reps (loadavg $(awk '{print $1}' /proc/loadavg)) ===" >&2
        for rep in $(seq 1 30); do run1 "stab32-v37" "$V37" 32 "$PINNED" "$rep"; done
        ;;
esac
