#!/usr/bin/env bash
# §36 flat-gap-bughunter round 2 measurement driver.
# Round-2 worktree (jitcode-refptr-bounce-14pct + dfg-osrexit-genlock-dclp-precheck
# + lazy-species-install-race sibling + §7.1a single-handoff + segwalk-copy +
# per-worker chunks + flatten1 parallelize). 3-rep medians, GIL-off pinned env.
set -u
...
RAW="$SB/results-v40ab-raw.jsonl"
: > "$RAW"
ulimit -c 0
...
# Wait for build-induced load to decay (synchronous; capped 90s).
