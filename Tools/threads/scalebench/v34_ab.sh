#!/usr/bin/env bash
# Run 3.4 A/B: d8ed7b6f5254 baseline vs v34 (campaign-3 + P0/R1/S2/S3 delta), same host, back-to-back per cell.
set -u
SB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE="$SB/../../../WebKitBuild/Release/bin/jsc-v33-baseline"
V34="$SB/../../../WebKitBuild/Release/bin/jsc-v34"
...
RAW="$SB/results-v34ab-raw.jsonl"
ulimit -c 0
...
run1() {
    ...
    echo "{\"label\":\"$label\",\"bin\":\"$(basename $bin)\",\"W\":$W,\"rep\":$rep,\"rc\":$rc,\"total_ms\":$total,\"rss_kb\":$rss,\"user_s\":$user,\"sys_s\":$syss,\"elapsed\":\"$elap\",\"cs\":\"$cs\",\"load\":\"$(awk '{print $1}' /proc/loadavg)\",\"json\":$json}" | tee -a "$RAW"
}
...
cell() {
    ...
    for rep in $(seq 1 $reps); do run1 "$label-v34" "$V34" "$W" "$envv" "$rep"; done
}
...
case "${1:-all}" in
    ...
    congc)
        echo "=== congc W=4 v34 only (loadavg $(awk '{print $1}' /proc/loadavg)) ===" >&2
        for rep in 1 2 3 4 5; do run1 "congc-w4-v34" "$V34" 4 "$PINNED JSC_useConcurrentSharedGCMarking=1" "$rep"; done
        ;;
esac
